 ...
 */
namespace S3IO\Aws2\Aws\Common\Model\MultipartUpload;

use S3IO\Aws2\Aws\Common\Client\AwsClientInterface;
use S3IO\Aws2\Aws\Common\Exception\MultipartUploadException;
 ...
 */
abstract class AbstractTransfer extends \S3IO\Aws2\Guzzle\Common\AbstractHasDispatcher implements \S3IO\Aws2\Aws\Common\Model\MultipartUpload\TransferInterface
{
     ...
     */
    public function __construct(\S3IO\Aws2\Aws\Common\Client\AwsClientInterface $client, \S3IO\Aws2\Aws\Common\Model\MultipartUpload\TransferStateInterface $state, \S3IO\Aws2\Guzzle\Http\EntityBody $source, array $options = array())
    {
        $this->client = $client;
        $this->state = $state;
        $this->source = $source;
    ...
    public function upload()
    {
        if ($this->state->isAborted()) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('The transfer has been aborted and cannot be uploaded');
        }
        $this->stopped = false;
        ...
        try {
            ...
            throw new \S3IO\Aws2\Aws\Common\Exception\MultipartUploadException($this->state, $e);
        }
        return $result;
    }
    /**
     ...
     * @return array
     */
    protected function getEventData(\S3IO\Aws2\Guzzle\Service\Command\OperationCommand $command = null)
    {
