...
abstract class AbstractUploadBuilder
{
     ...
     * @return $this
     */
    public function setClient(\S3IO\Aws2\Aws\Common\Client\AwsClientInterface $client)
    {
        $this->client = $client;
        return $this;
    }
    ...
    public function setSource($source)
    {
        ...
        if (is_string($source)) {
            if (!file_exists($source)) {
                throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException("File does not exist: {$source}");
            }
            // Clear the cache so that we send accurate file sizes
            clearstatcache(true, $source);
            $source = fopen($source, 'r');
        }
        $this->source = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($source);
        if ($this->source->isSeekable() && $this->source->getSize() == 0) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('Empty body provided to upload builder');
        }
        return $this;
    }
    /**
