 ...
 */
namespace S3IO\Aws2\Aws\Common\Credentials;

use S3IO\Aws2\Aws\Common\Enum\ClientOptions as Options;
use S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException;
use S3IO\Aws2\Aws\Common\Exception\RequiredExtensionNotLoadedException;
use S3IO\Aws2\Aws\Common\Exception\RuntimeException;
use S3IO\Aws2\Guzzle\Common\FromConfigInterface;
use S3IO\Aws2\Guzzle\Cache\CacheAdapterInterface;
use S3IO\Aws2\Guzzle\Cache\DoctrineCacheAdapter;
use S3IO\Aws2\Guzzle\Common\Collection;
/**
 * Basic implementation of the AWSCredentials interface that allows callers to
 * pass in the AWS access key and secret access in the constructor.
 */
class Credentials implements \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface, \S3IO\Aws2\Guzzle\Common\FromConfigInterface
{
    ...
    {
        return array(\S3IO\Aws2\Aws\Common\Enum\ClientOptions::KEY => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::SECRET => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN_TTD => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::PROFILE => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CACHE => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CACHE_KEY => null, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CLIENT => null);
    }
    /**
    ...
    public static function factory($config = array())
    {
        ...
        // Set up the cache
        $cache = $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CACHE];
        $cacheKey = $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CACHE_KEY] ?: 'credentials_' . ($config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::KEY] ?: crc32(gethostname()));
        if ($cacheKey && $cache instanceof CacheAdapterInterface && ($cached = self::createFromCache($cache, $cacheKey))) {
            return $cached;
        }
        // Create the credentials object
        if (!$config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::KEY] || !$config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::SECRET]) {
            $credentials = self::createFromEnvironment($config);
        } else {
            // Instantiate using short or long term credentials
            $credentials = new static($config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::KEY], $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::SECRET], $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN], $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN_TTD]);
        }
    ...
    {
        return json_encode(array(\S3IO\Aws2\Aws\Common\Enum\ClientOptions::KEY => $this->key, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::SECRET => $this->secret, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN => $this->token, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN_TTD => $this->ttd));
    }
    public function unserialize($serialized)
    {
        $data = json_decode($serialized, true);
        $this->key = $data[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::KEY];
        $this->secret = $data[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::SECRET];
        $this->token = $data[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN];
        $this->ttd = $data[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::TOKEN_TTD];
    }
    ...
    private static function createFromEnvironment($config)
    {
        ...
        try {
            ...
            return self::fromIni($config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::PROFILE]);
        } catch (\RuntimeException $e) {
            // Otherwise, try using instance profile credentials (available on EC2 instances)
            return new \S3IO\Aws2\Aws\Common\Credentials\RefreshableInstanceProfileCredentials(new static('', '', '', 1), $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CLIENT]);
        }
    }
    private static function createFromCache(\S3IO\Aws2\Guzzle\Cache\CacheAdapterInterface $cache, $cacheKey)
    {
        $cached = $cache->fetch($cacheKey);
        if ($cached instanceof CredentialsInterface && !$cached->isExpired()) {
            return new \S3IO\Aws2\Aws\Common\Credentials\CacheableCredentials($cached, $cache, $cacheKey);
        }
        return null;
    }
    private static function createCache(\S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials, $cache, $cacheKey)
    {
        if ($cache === 'true' || $cache === true) {
            // If no cache adapter was provided, then create one for the user
            // @codeCoverageIgnoreStart
            if (!extension_loaded('apc')) {
                throw new \S3IO\Aws2\Aws\Common\Exception\RequiredExtensionNotLoadedException('PHP has not been compiled with APC. Unable to cache ' . 'the credentials.');
            } elseif (!class_exists('S3IO\\Aws2\\Doctrine\\Common\\Cache\\ApcCache')) {
                throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('Cannot set ' . \S3IO\Aws2\Aws\Common\Enum\ClientOptions::CREDENTIALS_CACHE . ' to true because the Doctrine cache component is ' . 'not installed. Either install doctrine/cache or pass in an instantiated ' . 'S3IO\\Aws2\\Guzzle\\Cache\\CacheAdapterInterface object');
            }
            // @codeCoverageIgnoreEnd
            $cache = new \S3IO\Aws2\Guzzle\Cache\DoctrineCacheAdapter(new \S3IO\Aws2\Doctrine\Common\Cache\ApcCache());
        } elseif (!$cache instanceof CacheAdapterInterface) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('Unable to utilize caching with the specified options');
        }
        // Decorate the credentials with a cache
        return new \S3IO\Aws2\Aws\Common\Credentials\CacheableCredentials($credentials, $cache, $cacheKey);
    }
