 ...
 */
namespace S3IO\Aws2\Aws\Common\Signature;

use S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface;
use S3IO\Aws2\Aws\Common\Enum\DateFormat;
use S3IO\Aws2\Aws\Common\HostNameUtils;
...
use S3IO\Aws2\Guzzle\Stream\Stream;
/**
 * Signature Version 4
 * @link http://docs.aws.amazon.com/general/latest/gr/signature-version-4.html
 */
class SignatureV4 extends \S3IO\Aws2\Aws\Common\Signature\AbstractSignature implements \S3IO\Aws2\Aws\Common\Signature\EndpointSignatureInterface
{
    ...
    public function signRequest(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        $timestamp = $this->getTimestamp();
        $longDate = gmdate(\S3IO\Aws2\Aws\Common\Enum\DateFormat::ISO8601, $timestamp);
        ...
        if ($request->hasHeader('x-amz-date') || !$request->hasHeader('Date')) {
            ...
            $request->setHeader('Date', gmdate(\S3IO\Aws2\Aws\Common\Enum\DateFormat::RFC1123, $timestamp));
        }
        ...
        if (!$region || !$service) {
            $url = \S3IO\Aws2\Guzzle\Http\Url::factory($request->getUrl());
            $region = $region ?: \S3IO\Aws2\Aws\Common\HostNameUtils::parseRegionName($url);
            $service = $service ?: \S3IO\Aws2\Aws\Common\HostNameUtils::parseServiceName($url);
        }
    ...
    public function createPresignedUrl(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials, $expires)
    {
        $request = $this->createPresignedRequest($request, $credentials);
        $query = $request->getQuery();
        $httpDate = gmdate(\S3IO\Aws2\Aws\Common\Enum\DateFormat::ISO8601, $this->getTimestamp());
     ...
     */
    public static function convertPostToGet(\S3IO\Aws2\Guzzle\Http\Message\EntityEnclosingRequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $cloned = \S3IO\Aws2\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
     ...
     * @return string
     */
    protected function getPayload(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($request instanceof EntityEnclosingRequestInterface) {
            if ($request->getMethod() == 'POST' && count($request->getPostFields())) {
                ...
                return \S3IO\Aws2\Guzzle\Stream\Stream::getHash($request->getBody(), 'sha256');
            }
        }
     ...
     * @return string
     */
    protected function getPresignedPayload(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        return $this->getPayload($request);
    }
    protected function createCanonicalizedPath(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        ...
        if ($request instanceof EntityEnclosingRequestInterface && $request->getMethod() === 'POST' && strpos($request->getHeader('Content-Type'), 'application/x-www-form-urlencoded') === 0) {
            $sr = \S3IO\Aws2\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
            // Move POST fields to the query if they are present
            foreach ($request->getPostFields() as $name => $value) {
     ...
     *               - signed_headers
     */
    private function createSigningContext(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $payload)
    {
     ...
     * @return string
     */
    private function getCanonicalizedQueryString(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        foreach ($queryParams as $key => $values) {
            ...
            foreach ((array) $values as $value) {
                if ($value === \S3IO\Aws2\Guzzle\Http\QueryString::BLANK) {
                    $value = '';
                }
    ...
    private function addQueryValues($scope, \S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials, $expires)
    {
    ...
    private function moveHeadersToQuery(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $query = $request->getQuery();
