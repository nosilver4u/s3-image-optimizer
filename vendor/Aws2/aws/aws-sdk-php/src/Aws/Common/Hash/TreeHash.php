 ...
 */
namespace S3IO\Aws2\Aws\Common\Hash;

use S3IO\Aws2\Aws\Common\Enum\Size;
 ...
 */
class TreeHash implements \S3IO\Aws2\Aws\Common\Hash\ChunkHashInterface
{
    /**
    ...
    public static function fromChecksums(array $checksums, $inBinaryForm = false, $algorithm = self::DEFAULT_ALGORITHM)
    {
        ...
        $treeHash->checksums = $inBinaryForm ? $checksums : array_map('S3IO\\Aws2\\Aws\\Common\\Hash\\HashUtils::hexToBin', $checksums);
        // Pre-calculate hash
        $treeHash->getHash();
        return $treeHash;
    }
    ...
    public static function fromContent($content, $algorithm = self::DEFAULT_ALGORITHM)
    {
        ...
        $content = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($content);
        while ($data = $content->read(\S3IO\Aws2\Aws\Common\Enum\Size::MB)) {
    ...
    {
        \S3IO\Aws2\Aws\Common\Hash\HashUtils::validateAlgorithm($algorithm);
    ...
    public function addData($data)
    {
        ...
        if ($this->hash) {
            throw new \S3IO\Aws2\Aws\Common\Exception\LogicException('You may not add more data to a finalized tree hash.');
        }
        // Make sure that only 1MB chunks or smaller get passed in
        if (strlen($data) > \S3IO\Aws2\Aws\Common\Enum\Size::MB) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('The chunk of data added is too large for tree hashing.');
        }
    ...
    public function addChecksum($checksum, $inBinaryForm = false)
    {
        // Error if hash is already calculated
        if ($this->hash) {
            throw new \S3IO\Aws2\Aws\Common\Exception\LogicException('You may not add more checksums to a finalized tree hash.');
        }
        // Convert the checksum to binary form if necessary
        $this->checksums[] = $inBinaryForm ? $checksum : \S3IO\Aws2\Aws\Common\Hash\HashUtils::hexToBin($checksum);
        return $this;
    }
    /**
     * {@inheritdoc}
    ...
    public function getHash($returnBinaryForm = false)
    {
        if (!$this->hash) {
            ...
            $this->hash = \S3IO\Aws2\Aws\Common\Hash\HashUtils::binToHex($this->hashRaw);
        }
