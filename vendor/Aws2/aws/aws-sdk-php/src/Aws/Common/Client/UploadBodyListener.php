...
use S3IO\Aws2\Guzzle\Service\Command\AbstractCommand as Command;
 ...
 */
class UploadBodyListener implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    /**
     ...
     */
    public function onCommandBeforePrepare(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
        /** @var Command $command */
        ...
        if (in_array($command->getName(), $this->commands)) {
            ...
            if (null !== $body) {
                ...
                $command->set($this->bodyParameter, \S3IO\Aws2\Guzzle\Http\EntityBody::factory($body));
            } else {
                throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException("You must specify a non-null value for the {$this->bodyParameter} or {$this->sourceParameter} parameters.");
            }
        }
    }
}
