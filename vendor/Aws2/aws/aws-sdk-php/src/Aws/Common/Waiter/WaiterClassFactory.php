 ...
 */
class WaiterClassFactory implements \S3IO\Aws2\Aws\Common\Waiter\WaiterFactoryInterface
{
    /**
     ...
     */
    public function __construct($namespaces = array(), \S3IO\Aws2\Guzzle\Inflection\InflectorInterface $inflector = null)
    {
    ...
    public function build($waiter)
    {
        if (!($className = $this->getClassName($waiter))) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException("Waiter was not found matching {$waiter}.");
        }
