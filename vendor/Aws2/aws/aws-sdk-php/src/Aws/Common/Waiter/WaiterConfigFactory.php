 ...
 */
class WaiterConfigFactory implements \S3IO\Aws2\Aws\Common\Waiter\WaiterFactoryInterface
{
    /**
     ...
     */
    public function __construct(array $config, \S3IO\Aws2\Guzzle\Inflection\InflectorInterface $inflector = null)
    {
        $this->config = $config;
        $this->inflector = $inflector ?: \S3IO\Aws2\Guzzle\Inflection\Inflector::getDefault();
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \S3IO\Aws2\Aws\Common\Waiter\ConfigResourceWaiter($this->getWaiterConfig($waiter));
    }
    /**
     * {@inheritdoc}
     */
    ...
    protected function getWaiterConfig($name)
    {
        if (!$this->canBuild($name)) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('No waiter found matching "' . $name . '"');
        }
        // inflect the name if needed
        $name = isset($this->config[$name]) ? $name : $this->inflector->camel($name);
        $waiter = new \S3IO\Aws2\Aws\Common\Waiter\WaiterConfig($this->config[$name]);
        $waiter['name'] = $name;
        // Always use __default__ as the basis if it's set
        if (isset($this->config['__default__'])) {
            $parentWaiter = new \S3IO\Aws2\Aws\Common\Waiter\WaiterConfig($this->config['__default__']);
