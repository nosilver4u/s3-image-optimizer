 ...
 */
namespace S3IO\Aws2\Aws\Common\Iterator;
...
use S3IO\Aws2\Guzzle\Service\Resource\ResourceIterator;
/**
 * Iterate over a client command
 */
class AwsResourceIterator extends \S3IO\Aws2\Guzzle\Service\Resource\ResourceIterator
{
    /**
    ...
    protected function sendRequest()
    {
        do {
            ...
            $this->command->add(\S3IO\Aws2\Aws\Common\Enum\UaString::OPTION, \S3IO\Aws2\Aws\Common\Enum\UaString::ITERATOR);
    ...
    protected function handleResults(\S3IO\Aws2\Guzzle\Service\Resource\Model $result)
    {
        $results = array();
        ...
        if ($tokenParam = $this->get('input_token')) {
            ...
            if (is_array($tokenParam)) {
                if (is_array($this->nextToken) && count($tokenParam) === count($this->nextToken)) {
                    ...
                    throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('The definition of the iterator\'s token parameter and the actual token ' . 'value are not compatible.');
                }
            } else {
    ...
    protected function determineNextToken(\S3IO\Aws2\Guzzle\Service\Resource\Model $result)
    {
        $this->nextToken = null;
     ...
     * @return mixed|null
     */
    protected function getValueFromResult(\S3IO\Aws2\Guzzle\Service\Resource\Model $result, $key)
    {
