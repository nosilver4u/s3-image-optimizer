 ...
 */
class AwsResourceIteratorFactory implements \S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorFactoryInterface
{
    /**
     ...
     */
    public function __construct(array $config, \S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorFactoryInterface $primaryIteratorFactory = null)
    {
    ...
    public function build(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, array $options = array())
    {
        ...
        if ($this->primaryIteratorFactory && $this->primaryIteratorFactory->canBuild($command)) {
            ...
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException("Iterator was not found for {$commandName}.");
        } else {
            // Instantiate a generic AWS resource iterator
            $iterator = new \S3IO\Aws2\Aws\Common\Iterator\AwsResourceIterator($command, $options);
        }
        return $iterator;
    }
