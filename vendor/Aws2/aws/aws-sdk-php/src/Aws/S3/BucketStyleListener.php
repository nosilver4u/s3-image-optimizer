 ...
 */
class BucketStyleListener implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    ...
    public function onCommandAfterPrepare(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
        ...
        if (!$bucket) {
            ...
            $request->getParams()->set('s3.resource', '/' . rawurlencode($bucket) . ($key ? '/' . \S3IO\Aws2\Aws\S3\S3Client::encodeKey($key) : ''));
        } else {
