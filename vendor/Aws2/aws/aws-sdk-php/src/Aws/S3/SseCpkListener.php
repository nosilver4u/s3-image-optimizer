 ...
 */
class SseCpkListener implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    ...
    private function validateScheme(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command)
    {
        if ($command->getClient()->getConfig('scheme') !== 'https') {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('You must configure your S3 client to ' . 'use HTTPS in order to use the SSE-C features.');
        }
    }
    private function prepareSseParams(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, $isCopy = false)
    {
