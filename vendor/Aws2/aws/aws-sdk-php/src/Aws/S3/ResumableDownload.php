...
class ResumableDownload
{
     ...
     */
    public function __construct(\S3IO\Aws2\Aws\S3\S3Client $client, $bucket, $key, $target, array $params = array())
    {
        $this->params = $params;
        $this->client = $client;
        ...
        if (is_string($target)) {
            if (!($target = fopen($target, 'a+'))) {
                throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException("Unable to open {$target} for writing");
            }
            // Always append to the file
            fseek($target, 0, SEEK_END);
        }
        // Get the metadata and Content-MD5 of the object
        $this->target = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($target);
    }
    /**
    ...
    public function __invoke()
    {
        ...
        $this->params['SaveAs'] = new \S3IO\Aws2\Guzzle\Http\ReadLimitEntityBody($this->target, $this->meta['ContentLength'], $this->target->ftell());
    ...
    protected function checkIntegrity()
    {
        if ($this->target->isReadable() && ($expected = $this->meta['ContentMD5'])) {
            ...
            if ($actual != $expected) {
                throw new \S3IO\Aws2\Aws\Common\Exception\UnexpectedValueException("Message integrity check failed. Expected {$expected} but got {$actual}.");
            }
        }
    }
}
