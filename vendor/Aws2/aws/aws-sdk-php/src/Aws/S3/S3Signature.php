 ...
 */
class S3Signature implements \S3IO\Aws2\Aws\S3\S3SignatureInterface
{
    /**
    ...
    public function createPresignedUrl(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials, $expires)
    {
        ...
        $path = \S3IO\Aws2\Aws\S3\S3Client::encodeKey(rawurldecode($request->getPath()));
    ...
    public function signString($string, \S3IO\Aws2\Aws\Common\Credentials\CredentialsInterface $credentials)
    {
        return base64_encode(hash_hmac('sha1', $string, $credentials->getSecretKey(), true));
    }
    public function createCanonicalizedString(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $expires = null)
    {
     ...
     */
    private function createCanonicalizedAmzHeaders(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $headers = array();
        foreach ($request->getHeaders() as $name => $header) {
            $name = strtolower($name);
     ...
     * @return string
     */
    private function createCanonicalizedResource(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        foreach ($this->signableQueryString as $key) {
            if ($query->hasKey($key)) {
                ...
                if ($value !== '' && $value !== false && $value !== null && $value !== \S3IO\Aws2\Guzzle\Http\QueryString::BLANK) {
                    $buffer .= "={$value}";
                }
            }
        }
     ...
     * @return string
     */
    private function parseBucketName(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $baseUrl = \S3IO\Aws2\Guzzle\Http\Url::factory($request->getClient()->getBaseUrl());
