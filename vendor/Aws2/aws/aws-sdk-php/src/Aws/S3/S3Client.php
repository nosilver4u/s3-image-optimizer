...
use S3IO\Aws2\Aws\Common\Client\AbstractClient;
use S3IO\Aws2\Aws\Common\Client\ClientBuilder;
use S3IO\Aws2\Aws\Common\Client\ExpiredCredentialsChecker;
use S3IO\Aws2\Aws\Common\Client\UploadBodyListener;
...
use S3IO\Aws2\Aws\S3\Exception\AccessDeniedException;
use S3IO\Aws2\Aws\S3\Exception\Parser\S3ExceptionParser;
use S3IO\Aws2\Aws\S3\Exception\S3Exception;
use S3IO\Aws2\Aws\S3\Model\ClearBucket;
use S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer as AbstractMulti;
use S3IO\Aws2\Aws\S3\Model\MultipartUpload\UploadBuilder;
use S3IO\Aws2\Aws\S3\Sync\DownloadSyncBuilder;
use S3IO\Aws2\Aws\S3\Sync\UploadSyncBuilder;
use S3IO\Aws2\Guzzle\Common\Collection;
use S3IO\Aws2\Guzzle\Http\EntityBody;
use S3IO\Aws2\Guzzle\Http\Message\RequestInterface;
use S3IO\Aws2\Guzzle\Iterator\FilterIterator;
...
use S3IO\Aws2\Guzzle\Service\Command\Factory\AliasFactory;
use S3IO\Aws2\Guzzle\Service\Command\Factory\CompositeFactory;
use S3IO\Aws2\Guzzle\Service\Resource\Model;
use S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorInterface;
/**
 ...
 */
class S3Client extends \S3IO\Aws2\Aws\Common\Client\AbstractClient
{
    ...
    {
        $exceptionParser = new \S3IO\Aws2\Aws\S3\Exception\Parser\S3ExceptionParser();
        // Configure the custom exponential backoff plugin for retrying S3 specific errors
        if (!isset($config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::BACKOFF])) {
            ...
            $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::BACKOFF] = static::createBackoffPlugin($exceptionParser, $retries);
        }
        $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::SIGNATURE] = $signature = static::createSignature($config);
        $client = \S3IO\Aws2\Aws\Common\Client\ClientBuilder::factory(__NAMESPACE__)->setConfig($config)->setConfigDefaults(array(\S3IO\Aws2\Aws\Common\Enum\ClientOptions::VERSION => self::LATEST_API_VERSION, \S3IO\Aws2\Aws\Common\Enum\ClientOptions::SERVICE_DESCRIPTION => __DIR__ . '/Resources/s3-%s.php'))->setExceptionParser($exceptionParser)->setIteratorsConfig(array('more_key' => 'IsTruncated', 'operations' => array('ListBuckets', 'ListMultipartUploads' => array('limit_param' => 'MaxUploads', 'token_param' => array('KeyMarker', 'UploadIdMarker'), 'token_key' => array('NextKeyMarker', 'NextUploadIdMarker')), 'ListObjects' => array('limit_param' => 'MaxKeys', 'token_param' => 'Marker', 'token_key' => 'NextMarker'), 'ListObjectVersions' => array('limit_param' => 'MaxKeys', 'token_param' => array('KeyMarker', 'VersionIdMarker'), 'token_key' => array('nextKeyMarker', 'nextVersionIdMarker')), 'ListParts' => array('limit_param' => 'MaxParts', 'result_key' => 'Parts', 'token_param' => 'PartNumberMarker', 'token_key' => 'NextPartNumberMarker'))))->build();
        // Use virtual hosted buckets when possible
        $client->addSubscriber(new \S3IO\Aws2\Aws\S3\BucketStyleListener());
        // Ensure that ACP headers are applied when needed
        $client->addSubscriber(new \S3IO\Aws2\Aws\S3\AcpListener());
        // Validate and add required Content-MD5 hashes (e.g. DeleteObjects)
        $client->addSubscriber(new \S3IO\Aws2\Aws\S3\S3Md5Listener($signature));
        // Allow for specifying bodies with file paths and file handles
        $client->addSubscriber(new \S3IO\Aws2\Aws\Common\Client\UploadBodyListener(array('PutObject', 'UploadPart')));
        // Ensures that if a SSE-CPK key is provided, the key and md5 are formatted correctly
        $client->addSubscriber(new \S3IO\Aws2\Aws\S3\SseCpkListener());
        // Add aliases for some S3 operations
        $default = \S3IO\Aws2\Guzzle\Service\Command\Factory\CompositeFactory::getDefaultChain($client);
        $default->add(new \S3IO\Aws2\Guzzle\Service\Command\Factory\AliasFactory($client, static::$commandAliases), 'S3IO\\Aws2\\Guzzle\\Service\\Command\\Factory\\ServiceDescriptionFactory');
     ...
     */
    private static function createBackoffPlugin(\S3IO\Aws2\Aws\S3\Exception\Parser\S3ExceptionParser $exceptionParser, $retries = 3)
    {
        return new \S3IO\Aws2\Guzzle\Plugin\Backoff\BackoffPlugin(new \S3IO\Aws2\Guzzle\Plugin\Backoff\TruncatedBackoffStrategy($retries, new \S3IO\Aws2\Aws\S3\IncompleteMultipartUploadChecker(new \S3IO\Aws2\Guzzle\Plugin\Backoff\CurlBackoffStrategy(null, new \S3IO\Aws2\Guzzle\Plugin\Backoff\HttpBackoffStrategy(null, new \S3IO\Aws2\Aws\S3\SocketTimeoutChecker(new \S3IO\Aws2\Aws\Common\Client\ExpiredCredentialsChecker($exceptionParser, new \S3IO\Aws2\Guzzle\Plugin\Backoff\ExponentialBackoffStrategy())))))));
    }
    /**
    ...
    {
        $currentValue = isset($config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::SIGNATURE]) ? $config[\S3IO\Aws2\Aws\Common\Enum\ClientOptions::SIGNATURE] : null;
        ...
        if ($currentValue == 'v4' || $requiresV4) {
            ...
            $currentValue = new \S3IO\Aws2\Aws\S3\S3SignatureV4('s3');
        } elseif (!$currentValue || $currentValue == 's3') {
            // Use the Amazon S3 signature by default
            $currentValue = new \S3IO\Aws2\Aws\S3\S3Signature();
        }
        // A region is require with v4
        if ($currentValue instanceof SignatureV4 && !isset($config['region'])) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('A region must be specified ' . 'when using signature version 4');
        }
        return $currentValue;
    }
    /**
     ...
     */
    public function createPresignedUrl(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $expires)
    {
        if ($request->getClient() !== $this) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('The request object must be associated with the client. Use the ' . '$client->get(), $client->head(), $client->post(), $client->put(), etc. methods when passing in a ' . 'request object');
        }
    ...
    {
        $clear = new \S3IO\Aws2\Aws\S3\Model\ClearBucket($this, $bucket);
        return $clear->clear();
    }
    /**
    ...
    {
        \S3IO\Aws2\Aws\S3\StreamWrapper::register($this);
        return $this;
    }
    /**
    ...
    {
        $body = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($body);
        $options = \S3IO\Aws2\Guzzle\Common\Collection::fromConfig(array_change_key_case($options), array('min_part_size' => \S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer::MIN_PART_SIZE, 'params' => array(), 'concurrency' => $body->getWrapper() == 'plainfile' ? 3 : 1));
        ...
        $transfer = \S3IO\Aws2\Aws\S3\Model\MultipartUpload\UploadBuilder::newInstance()->setBucket($bucket)->setKey($key)->setMinPartSize($options['min_part_size'])->setConcurrency($options['concurrency'])->setClient($this)->setSource($body)->setTransferOptions($options->toArray())->addOptions($options['params'])->setOption('ACL', $acl)->build();
        if ($options['before_upload']) {
            $transfer->getEventDispatcher()->addListener(\S3IO\Aws2\Aws\Common\Model\MultipartUpload\AbstractTransfer::BEFORE_PART_UPLOAD, $options['before_upload']);
        }
    ...
    {
        $options = \S3IO\Aws2\Guzzle\Common\Collection::fromConfig($options, array('base_dir' => realpath($directory) ?: $directory));
        $builder = $options['builder'] ?: \S3IO\Aws2\Aws\S3\Sync\UploadSyncBuilder::getInstance();
    ...
    {
        $options = new \S3IO\Aws2\Guzzle\Common\Collection($options);
        $builder = $options['builder'] ?: \S3IO\Aws2\Aws\S3\Sync\DownloadSyncBuilder::getInstance();
    ...
    {
        if (!$prefix && !$regex) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('A prefix or regex is required, or use S3Client::clearBucket().');
        }
        $clear = new \S3IO\Aws2\Aws\S3\Model\ClearBucket($this, $bucket);
        $iterator = $this->getIterator('ListObjects', array('Bucket' => $bucket, 'Prefix' => $prefix));
        if ($regex) {
            $iterator = new \S3IO\Aws2\Guzzle\Iterator\FilterIterator($iterator, function ($current) use($regex) {
                return preg_match($regex, $current['Key']);
            });
        }
        $clear->setIterator($iterator);
        if (isset($options['before_delete'])) {
            $clear->getEventDispatcher()->addListener(\S3IO\Aws2\Aws\S3\Model\ClearBucket::BEFORE_CLEAR, $options['before_delete']);
        }
        return $clear->clear();
    }
    /**
     ...
     */
    protected function checkExistenceWithCommand(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, $accept403 = false)
    {
        try {
            $command->execute();
            $exists = true;
