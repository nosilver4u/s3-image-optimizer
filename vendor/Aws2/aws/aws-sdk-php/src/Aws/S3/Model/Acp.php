...
use S3IO\Aws2\Guzzle\Service\Command\AbstractCommand;
/**
 * Amazon S3 Access Control Policy (ACP)
 */
class Acp implements \S3IO\Aws2\Guzzle\Common\ToArrayInterface, \IteratorAggregate, \Countable
{
    /**
     ...
     */
    public function __construct(\S3IO\Aws2\Aws\S3\Model\Grantee $owner, $grants = null)
    {
        $this->setOwner($owner);
    ...
    {
        $builder = new \S3IO\Aws2\Aws\S3\Model\AcpBuilder();
     ...
     */
    public function setOwner(\S3IO\Aws2\Aws\S3\Model\Grantee $owner)
    {
        if (!$owner->isCanonicalUser()) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('The owner must have an ID set.');
        }
        $this->owner = $owner;
        return $this;
    }
    ...
    public function setGrants($grants = array())
    {
        ...
        if ($grants) {
            if (is_array($grants) || $grants instanceof \Traversable) {
                ...
                throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('Grants must be passed in as an array or Traversable object.');
            }
        }
        return $this;
    }
     ...
     * @return $this
     */
    public function addGrant(\S3IO\Aws2\Aws\S3\Model\Grant $grant)
    {
        if (count($this->grants) < 100) {
            $this->grants->attach($grant);
        } else {
            throw new \S3IO\Aws2\Aws\Common\Exception\OverflowException('An ACP may contain up to 100 grants.');
        }
        return $this;
    }
    /**
     ...
     * @return $this
     */
    public function updateCommand(\S3IO\Aws2\Guzzle\Service\Command\AbstractCommand $command)
    {
        $parameters = array();
