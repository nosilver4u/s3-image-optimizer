...
use S3IO\Aws2\Aws\Common\Model\MultipartUpload\AbstractUploadBuilder;
 ...
 */
class UploadBuilder extends \S3IO\Aws2\Aws\Common\Model\MultipartUpload\AbstractUploadBuilder
{
    /**
     ...
     */
    protected $minPartSize = \S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer::MIN_PART_SIZE;
    /**
    ...
    {
        $this->minPartSize = (int) max((int) $minSize, \S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer::MIN_PART_SIZE);
        return $this;
    }
    /**
    ...
    public function build()
    {
        ...
        if (!isset($this->commandOptions['Bucket']) || !isset($this->commandOptions['Key']) || !$this->client || !$this->source) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('You must specify a Bucket, Key, client, and source.');
        }
        if ($this->state && !$this->source->isSeekable()) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('You cannot resume a transfer using a non-seekable source.');
        }
        // If no state was set, then create one by initiating or loading a multipart upload
        if (is_string($this->state)) {
            $this->state = \S3IO\Aws2\Aws\S3\Model\MultipartUpload\TransferState::fromUploadId($this->client, \S3IO\Aws2\Aws\S3\Model\MultipartUpload\UploadId::fromParams(array('Bucket' => $this->commandOptions['Bucket'], 'Key' => $this->commandOptions['Key'], 'UploadId' => $this->state)));
        } elseif (!$this->state) {
        ...
        return $this->concurrency > 1 ? new \S3IO\Aws2\Aws\S3\Model\MultipartUpload\ParallelTransfer($this->client, $this->state, $this->source, $options) : new \S3IO\Aws2\Aws\S3\Model\MultipartUpload\SerialTransfer($this->client, $this->state, $this->source, $options);
    }
    /**
     * {@inheritdoc}
     */
    protected function initiateMultipartUpload()
    {
        ...
        $params = array_replace(array(\S3IO\Aws2\Aws\Common\Enum\UaString::OPTION => \S3IO\Aws2\Aws\Common\Enum\UaString::MULTIPART_UPLOAD, 'command.headers' => $this->headers, 'Metadata' => array()), $this->commandOptions);
        ...
        return new \S3IO\Aws2\Aws\S3\Model\MultipartUpload\TransferState(\S3IO\Aws2\Aws\S3\Model\MultipartUpload\UploadId::fromParams($params));
    }
}
