...
use S3IO\Aws2\Guzzle\Http\ReadLimitEntityBody;
/**
 * Transfers multipart upload parts in parallel
 */
class ParallelTransfer extends \S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer
{
    /**
     * {@inheritdoc}
     */
    protected function init()
    {
        ...
        if (!$this->source->isLocal() || $this->source->getWrapper() != 'plainfile') {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('The source data must be a local file stream when uploading in parallel.');
        }
        if (empty($this->options['concurrency'])) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('The `concurrency` option must be specified when instantiating.');
        }
    }
    /**
     * {@inheritdoc}
    ...
    protected function transfer()
    {
        ...
        while (!$this->stopped && count($this->state) < $totalParts) {
            ...
            for ($i = 0; $i < $concurrency && $i + $currentTotal < $totalParts; $i++) {
                ...
                $eventData['command'] = $this->client->getCommand('UploadPart', array_replace($params, array('PartNumber' => count($this->state) + 1 + $i, 'Body' => $partsToSend[$i], 'ContentMD5' => (bool) $this->options['part_md5'], \S3IO\Aws2\Aws\Common\Enum\UaString::OPTION => \S3IO\Aws2\Aws\Common\Enum\UaString::MULTIPART_UPLOAD)));
            ...
            foreach ($this->client->execute($commands) as $command) {
                $this->state->addPart(\S3IO\Aws2\Aws\S3\Model\MultipartUpload\UploadPart::fromArray(array('PartNumber' => $command['PartNumber'], 'ETag' => $command->getResponse()->getEtag(), 'Size' => (int) $command->getRequest()->getBody()->getContentLength(), 'LastModified' => gmdate(\S3IO\Aws2\Aws\Common\Enum\DateFormat::RFC2822))));
    ...
    protected function prepareParts($concurrency)
    {
        ...
        $parts = array(new \S3IO\Aws2\Guzzle\Http\ReadLimitEntityBody($this->source, $this->partSize));
        // Open EntityBody handles for each part to upload in parallel
        for ($i = 1; $i < $concurrency; $i++) {
            $parts[] = new \S3IO\Aws2\Guzzle\Http\ReadLimitEntityBody(new \S3IO\Aws2\Guzzle\Http\EntityBody(fopen($url, 'r')), $this->partSize);
        }
        return $parts;
    }
}
