...
use S3IO\Aws2\Aws\Common\Client\AwsClientInterface;
use S3IO\Aws2\Aws\Common\Exception\OverflowException;
use S3IO\Aws2\Aws\Common\Enum\UaString as Ua;
use S3IO\Aws2\Aws\S3\Exception\InvalidArgumentException;
use S3IO\Aws2\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 * single request
 */
class DeleteObjectsTransfer implements \S3IO\Aws2\Guzzle\Batch\BatchTransferInterface
{
    /**
     ...
     */
    public function __construct(\S3IO\Aws2\Aws\Common\Client\AwsClientInterface $client, $bucket, $mfa = null)
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->mfa = $mfa;
    ...
    public function transfer(array $batch)
    {
        ...
        if (count($batch) > 1000) {
            throw new \S3IO\Aws2\Aws\Common\Exception\OverflowException('Batches should be divided into chunks of no larger than 1000 keys');
        }
        $del = array();
        $command = $this->client->getCommand('DeleteObjects', array('Bucket' => $this->bucket, \S3IO\Aws2\Aws\Common\Enum\UaString::OPTION => \S3IO\Aws2\Aws\Common\Enum\UaString::BATCH));
        if ($this->mfa) {
        ...
        foreach ($batch as $object) {
            ...
            if (!is_array($object) || !isset($object['Key'])) {
                throw new \S3IO\Aws2\Aws\S3\Exception\InvalidArgumentException('Invalid batch item encountered: ' . var_export($batch, true));
            }
     ...
     */
    protected function processResponse(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command)
    {
        $result = $command->getResult();
        // Ensure that the objects were deleted successfully
        if (!empty($result['Errors'])) {
            $errors = $result['Errors'];
            throw new \S3IO\Aws2\Aws\S3\Exception\DeleteMultipleObjectsException($errors);
        }
    }
}
