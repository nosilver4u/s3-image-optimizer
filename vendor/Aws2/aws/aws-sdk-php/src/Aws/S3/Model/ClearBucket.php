...
use S3IO\Aws2\Guzzle\Batch\FlushingBatch;
use S3IO\Aws2\Guzzle\Batch\ExceptionBufferingBatch;
use S3IO\Aws2\Guzzle\Batch\NotifyingBatch;
 ...
 */
class ClearBucket extends \S3IO\Aws2\Guzzle\Common\AbstractHasDispatcher
{
    /**
     ...
     */
    public function __construct(\S3IO\Aws2\Aws\Common\Client\AwsClientInterface $client, $bucket)
    {
        $this->client = $client;
        $this->bucket = $bucket;
    }
    ...
    {
        $that = $this;
        $batch = \S3IO\Aws2\Aws\S3\Model\DeleteObjectsBatch::factory($this->client, $this->bucket, $this->mfa);
        $batch = new \S3IO\Aws2\Guzzle\Batch\NotifyingBatch($batch, function ($items) use($that) {
            $that->dispatch(\S3IO\Aws2\Aws\S3\Model\ClearBucket::AFTER_DELETE, array('keys' => $items));
        });
        $batch = new \S3IO\Aws2\Guzzle\Batch\FlushingBatch(new \S3IO\Aws2\Guzzle\Batch\ExceptionBufferingBatch($batch), 1000);
        ...
        if (count($batch->getExceptions())) {
            $e = new \S3IO\Aws2\Guzzle\Common\Exception\ExceptionCollection();
