 ...
 */
class PostObject extends \S3IO\Aws2\Guzzle\Common\Collection
{
    /**
     ...
     * @param array $options
     */
    public function __construct(\S3IO\Aws2\Aws\S3\S3Client $client, $bucket, array $options = array())
    {
    ...
    public function prepareData()
    {
        ...
        $options = \S3IO\Aws2\Guzzle\Common\Collection::fromConfig($this->data, array('ttd' => '+1 hour', 'key' => '^${filename}'));
        // Format ttd option
        $ttd = $options['ttd'];
        ...
        $policy = array('expiration' => gmdate(\S3IO\Aws2\Aws\Common\Enum\DateFormat::ISO8601_S3, $ttd), 'conditions' => array(array('bucket' => $this->bucket)));
        // Configure the endpoint/action
        $url = \S3IO\Aws2\Guzzle\Http\Url::factory($this->client->getBaseUrl());
