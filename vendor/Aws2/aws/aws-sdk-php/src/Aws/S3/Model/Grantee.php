 ...
 */
namespace S3IO\Aws2\Aws\S3\Model;

use S3IO\Aws2\Aws\S3\Enum\Group;
use S3IO\Aws2\Aws\S3\Enum\GranteeType;
use S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException;
use S3IO\Aws2\Aws\Common\Exception\UnexpectedValueException;
use S3IO\Aws2\Aws\Common\Exception\LogicException;
use S3IO\Aws2\Guzzle\Common\ToArrayInterface;
/**
 * Amazon S3 Grantee model
 */
class Grantee implements \S3IO\Aws2\Guzzle\Common\ToArrayInterface
{
    /**
     * @var array A map of grantee types to grant header value prefixes
     */
    protected static $headerMap = array(\S3IO\Aws2\Aws\S3\Enum\GranteeType::USER => 'id', \S3IO\Aws2\Aws\S3\Enum\GranteeType::EMAIL => 'emailAddress', \S3IO\Aws2\Aws\S3\Enum\GranteeType::GROUP => 'uri');
    /**
    ...
    {
        $this->type = \S3IO\Aws2\Aws\S3\Enum\GranteeType::USER;
    ...
    {
        if (in_array($id, \S3IO\Aws2\Aws\S3\Enum\Group::values())) {
            $this->type = \S3IO\Aws2\Aws\S3\Enum\GranteeType::GROUP;
        } elseif (!is_string($id)) {
            throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('The grantee ID must be provided as a string value.');
        }
        if (strpos($id, '@') !== false) {
            $this->type = \S3IO\Aws2\Aws\S3\Enum\GranteeType::EMAIL;
        }
        if ($expectedType && $expectedType !== $this->type) {
            throw new \S3IO\Aws2\Aws\Common\Exception\UnexpectedValueException('The type of the grantee after ' . 'setting the ID did not match the specified, expected type "' . $expectedType . '" but received "' . $this->type . '".');
        }
        $this->id = $id;
        return $this;
    }
    ...
    {
        if ($this->type === \S3IO\Aws2\Aws\S3\Enum\GranteeType::USER) {
            ...
            if ($displayName) {
                throw new \S3IO\Aws2\Aws\Common\Exception\LogicException('The display name can only be set ' . 'for grantees specified by ID.');
            }
        }
        return $this;
    }
    ...
    {
        return $this->type === \S3IO\Aws2\Aws\S3\Enum\GranteeType::USER;
    }
    /**
    ...
    {
        return $this->type === \S3IO\Aws2\Aws\S3\Enum\GranteeType::EMAIL;
    }
    /**
    ...
    {
        return $this->type === \S3IO\Aws2\Aws\S3\Enum\GranteeType::GROUP;
    }
    /**
    ...
    public function toArray()
    {
        ...
        switch ($this->type) {
            case \S3IO\Aws2\Aws\S3\Enum\GranteeType::USER:
                $result['ID'] = $this->id;
                $result['DisplayName'] = $this->displayName;
                break;
            case \S3IO\Aws2\Aws\S3\Enum\GranteeType::EMAIL:
                $result['EmailAddress'] = $this->id;
                break;
            case \S3IO\Aws2\Aws\S3\Enum\GranteeType::GROUP:
                $result['URI'] = $this->id;
        }
        return $result;
    }
