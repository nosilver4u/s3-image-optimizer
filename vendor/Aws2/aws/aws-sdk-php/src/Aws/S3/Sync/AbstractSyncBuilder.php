...
use S3IO\Aws2\Aws\S3\S3Client;
use S3IO\Aws2\Aws\S3\Iterator\OpendirIterator;
...
abstract class AbstractSyncBuilder
{
     ...
     * @return $this
     */
    public function setClient(\S3IO\Aws2\Aws\S3\S3Client $client)
    {
        $this->client = $client;
        return $this;
    }
     ...
     * @return $this
     */
    public function setSourceFilenameConverter(\S3IO\Aws2\Aws\S3\Sync\FilenameConverterInterface $converter)
    {
     ...
     * @return $this
     */
    public function setTargetFilenameConverter(\S3IO\Aws2\Aws\S3\Sync\FilenameConverterInterface $converter)
    {
    ...
    public function addRegexFilter($search)
    {
        ...
        $this->sourceIterator = new \S3IO\Aws2\Guzzle\Iterator\FilterIterator($this->sourceIterator, function ($i) use($search) {
    ...
    public function build()
    {
        ...
        if (!$this->forcing) {
            $this->sourceIterator->rewind();
            $this->sourceIterator = new \S3IO\Aws2\Aws\S3\Sync\ChangedFilesIterator(new \NoRewindIterator($this->sourceIterator), $this->getTargetIterator(), $this->sourceConverter, $this->targetConverter);
            $this->sourceIterator->rewind();
        }
     ...
     */
    protected abstract function addDebugListener(\S3IO\Aws2\Aws\S3\Sync\AbstractSync $sync, $resource);
    /**
    ...
    {
        if (!$this->client) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('No client was provided');
        }
        if (!$this->bucket) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('No bucket was provided');
        }
        $this->assertFileIteratorSet();
    }
    /**
    ...
    protected function assertFileIteratorSet()
    {
        ...
        if (!isset($this->sourceIterator)) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('A source file iterator must be specified');
        }
    }
    /**
    ...
    {
        $f = new \S3IO\Aws2\Guzzle\Iterator\FilterIterator($iterator, function ($i) {
            if (!$i instanceof \SplFileInfo) {
                throw new \S3IO\Aws2\Aws\Common\Exception\UnexpectedValueException('All iterators for UploadSync must return SplFileInfo objects');
            }
            return $i->isFile();
        });
        $f->rewind();
    ...
    {
        $params = $this->params;
        $sync->getEventDispatcher()->addListener(\S3IO\Aws2\Aws\S3\Sync\UploadSync::BEFORE_TRANSFER, function (\S3IO\Aws2\Guzzle\Common\Event $e) use($params) {
    ...
    protected function createS3Iterator()
    {
        ...
        return $this->filterIterator(new \NoRewindIterator(new \S3IO\Aws2\Aws\S3\Iterator\OpendirIterator($dh, $dir)));
    }
}
