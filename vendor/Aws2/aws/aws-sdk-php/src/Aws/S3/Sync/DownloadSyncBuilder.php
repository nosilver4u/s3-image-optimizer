...
use S3IO\Aws2\Aws\S3\ResumableDownload;
...
class DownloadSyncBuilder extends \S3IO\Aws2\Aws\S3\Sync\AbstractSyncBuilder
{
    /** @var bool */
    ...
    {
        $sync = new \S3IO\Aws2\Aws\S3\Sync\DownloadSync(array('client' => $this->client, 'bucket' => $this->bucket, 'iterator' => $this->sourceIterator, 'source_converter' => $this->sourceConverter, 'target_converter' => $this->targetConverter, 'concurrency' => $this->concurrency, 'resumable' => $this->resumable, 'directory' => $this->directory));
        return $sync;
    }
    protected function getTargetIterator()
    {
        if (!$this->directory) {
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('A directory is required');
        }
        if (!is_dir($this->directory) && !mkdir($this->directory, 0777, true)) {
            // @codeCoverageIgnoreStart
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('Unable to create root download directory: ' . $this->directory);
    ...
    {
        return new \S3IO\Aws2\Aws\S3\Sync\KeyConverter("s3://{$this->bucket}/{$this->baseDir}", $this->directory . DIRECTORY_SEPARATOR, $this->delimiter);
    }
    protected function getDefaultTargetConverter()
    {
        return new \S3IO\Aws2\Aws\S3\Sync\KeyConverter("s3://{$this->bucket}/{$this->baseDir}", '', $this->delimiter);
    }
