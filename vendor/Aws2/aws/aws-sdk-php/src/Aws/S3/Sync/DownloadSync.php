 ...
 */
class DownloadSync extends \S3IO\Aws2\Aws\S3\Sync\AbstractSync
{
    protected function createTransferAction(\SplFileInfo $file)
    {
        ...
        if (file_exists($filename) && $this->options['resumable']) {
            return new \S3IO\Aws2\Aws\S3\ResumableDownload($this->options['client'], $bucket, $key, $filename);
        }
    ...
    protected function createDirectory($filename)
    {
        ...
        if (!is_dir($directory) && !mkdir($directory, 0777, true)) {
            ...
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('Could not create directory: ' . $directory . ' - ' . $errors['message']);
        }
    }
