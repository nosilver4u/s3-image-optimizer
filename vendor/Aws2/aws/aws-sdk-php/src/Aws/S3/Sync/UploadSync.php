...
use S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer;
use S3IO\Aws2\Guzzle\Http\EntityBody;
/**
 * Uploads a local directory tree to Amazon S3
 */
class UploadSync extends \S3IO\Aws2\Aws\S3\Sync\AbstractSync
{
    const BEFORE_MULTIPART_BUILD = 's3.sync.before_multipart_build';
    protected function init()
    {
        if (null == $this->options['multipart_upload_size']) {
            $this->options['multipart_upload_size'] = \S3IO\Aws2\Aws\S3\Model\MultipartUpload\AbstractTransfer::MIN_PART_SIZE;
        }
    }
    protected function createTransferAction(\SplFileInfo $file)
    {
        ...
        if (!($resource = fopen($filename, 'r'))) {
            ...
            throw new \S3IO\Aws2\Aws\Common\Exception\RuntimeException('Could not open ' . $file->getPathname() . ' for reading');
            // @codeCoverageIgnoreEnd
        }
        $key = $this->options['source_converter']->convert($filename);
        $body = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($resource);
        ...
        if ($body->getWrapper() == 'plainfile' && $file->getSize() >= $this->options['multipart_upload_size']) {
            $builder = \S3IO\Aws2\Aws\S3\Model\MultipartUpload\UploadBuilder::newInstance()->setBucket($this->options['bucket'])->setKey($key)->setMinPartSize($this->options['multipart_upload_size'])->setOption($aclType, $acl)->setClient($this->options['client'])->setSource($body)->setConcurrency($this->options['concurrency']);
