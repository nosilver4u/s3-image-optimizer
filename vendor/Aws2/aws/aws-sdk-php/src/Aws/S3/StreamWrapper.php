...
use S3IO\Aws2\Aws\S3\Exception\NoSuchKeyException;
use S3IO\Aws2\Aws\S3\Iterator\ListObjectsIterator;
use S3IO\Aws2\Guzzle\Http\EntityBody;
use S3IO\Aws2\Guzzle\Http\CachingEntityBody;
use S3IO\Aws2\Guzzle\Http\Mimetypes;
use S3IO\Aws2\Guzzle\Iterator\FilterIterator;
use S3IO\Aws2\Guzzle\Stream\PhpStreamRequestFactory;
...
class StreamWrapper
{
     ...
     */
    public static function register(\S3IO\Aws2\Aws\S3\S3Client $client)
    {
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \S3IO\Aws2\Guzzle\Http\Mimetypes::getInstance()->fromFilename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = new \S3IO\Aws2\Guzzle\Iterator\FilterIterator($objectIterator, function ($key) use($filterFn) {
    ...
    protected function openReadStream(array $params, array &$errors)
    {
        ...
        $factory = $this->getOption('stream_factory') ?: new \S3IO\Aws2\Guzzle\Stream\PhpStreamRequestFactory();
        $this->body = $factory->fromRequest($request, array(), array('stream_class' => 'S3IO\\Aws2\\Guzzle\\Http\\EntityBody'));
        ...
        if ($this->getOption('seekable')) {
            $this->body = new \S3IO\Aws2\Guzzle\Http\CachingEntityBody($this->body);
        }
        return true;
    }
    /**
    ...
    {
        $this->body = new \S3IO\Aws2\Guzzle\Http\EntityBody(fopen('php://temp', 'r+'));
        return true;
    }
    /**
