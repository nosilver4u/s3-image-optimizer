 ...
 */
class AcpListener implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    /**
     * {@inheritdoc}
     */
        ...
        if ($operation->hasParam('ACP') && $command->hasKey('ACP')) {
            if ($acp = $command->get('ACP')) {
                ...
                if (!$acp instanceof Acp) {
                    throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('ACP must be an instance of Aws\\S3\\Model\\Acp');
                }
                // Check if the user specified both an ACP and Grants
                if ($command->hasKey('Grants')) {
                    throw new \S3IO\Aws2\Aws\Common\Exception\InvalidArgumentException('Use either the ACP parameter or the Grants parameter. Do not use both.');
                }
