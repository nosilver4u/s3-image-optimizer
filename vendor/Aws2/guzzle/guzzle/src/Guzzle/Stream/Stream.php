 ...
 */
class Stream implements \S3IO\Aws2\Guzzle\Stream\StreamInterface
{
     ...
     */
    public static function getHash(\S3IO\Aws2\Guzzle\Stream\StreamInterface $stream, $algo, $rawOutput = false)
    {
        $pos = $stream->ftell();
        if (!$stream->seek(0)) {
            return false;
    ...
    public function setStream($stream, $size = null)
    {
        if (!is_resource($stream)) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Stream must be a resource');
        }
        $this->size = $size;
        $this->stream = $stream;
        $this->rebuildCache();
