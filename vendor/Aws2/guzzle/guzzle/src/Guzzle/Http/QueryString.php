...
use S3IO\Aws2\Guzzle\Http\QueryAggregator\QueryAggregatorInterface;
use S3IO\Aws2\Guzzle\Http\QueryAggregator\PhpAggregator;
/**
 * Query string object to handle managing query string parameters and aggregating those parameters together as a string.
 */
class QueryString extends \S3IO\Aws2\Guzzle\Common\Collection
{
    ...
    public static function fromString($query)
    {
        ...
        if ($foundDuplicates && !$foundPhpStyle) {
            $q->setAggregator(new \S3IO\Aws2\Guzzle\Http\QueryAggregator\DuplicateAggregator());
        }
        return $q;
    }
    /**
     ...
     */
    public function setAggregator(\S3IO\Aws2\Guzzle\Http\QueryAggregator\QueryAggregatorInterface $aggregator = null)
    {
        // Use the default aggregator if none was set
        if (!$aggregator) {
            if (!self::$defaultAggregator) {
                self::$defaultAggregator = new \S3IO\Aws2\Guzzle\Http\QueryAggregator\PhpAggregator();
            }
