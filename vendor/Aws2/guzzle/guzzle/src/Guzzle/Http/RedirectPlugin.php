...
use S3IO\Aws2\Guzzle\Http\Exception\TooManyRedirectsException;
use S3IO\Aws2\Guzzle\Http\Exception\CouldNotRewindStreamException;
use S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface;
/**
 * Plugin to implement HTTP redirects. Can redirect like a web browser or using strict RFC 2616 compliance
 */
class RedirectPlugin implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function cleanupRequest(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
     ...
     */
    protected function getOriginalRequest(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $original = $request;
     ...
     */
    protected function createRedirectRequest(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $statusCode, $location, \S3IO\Aws2\Guzzle\Http\Message\RequestInterface $original)
    {
        ...
        if ($request instanceof EntityEnclosingRequestInterface && ($statusCode == 303 || !$strict && $statusCode <= 302)) {
            $redirectRequest = \S3IO\Aws2\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
        } else {
        ...
        $location = \S3IO\Aws2\Guzzle\Http\Url::factory($location);
        ...
        $redirectRequest->getEventDispatcher()->addListener('request.before_send', $func = function ($e) use(&$func, $request, $redirectRequest) {
            ...
            $e['request']->getParams()->set(\S3IO\Aws2\Guzzle\Http\RedirectPlugin::PARENT_REQUEST, $request);
        });
     ...
     * @return RequestInterface
     */
    protected function prepareRedirection(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $original, \S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Guzzle\Http\Message\Response $response)
    {
     ...
     */
    protected function sendRedirectRequest(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $original, \S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Guzzle\Http\Message\Response $response)
    {
     ...
     */
    protected function throwTooManyRedirectsException(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $original, $max)
    {
        $original->getEventDispatcher()->addListener('request.complete', $func = function ($e) use(&$func, $original, $max) {
            ...
            throw new \S3IO\Aws2\Guzzle\Http\Exception\TooManyRedirectsException($str);
        });
    }
}
