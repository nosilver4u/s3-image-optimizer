...
use S3IO\Aws2\Guzzle\Http\Exception\BadResponseException;
use S3IO\Aws2\Guzzle\Http\ClientInterface;
use S3IO\Aws2\Guzzle\Http\EntityBody;
use S3IO\Aws2\Guzzle\Http\EntityBodyInterface;
use S3IO\Aws2\Guzzle\Http\Message\Header\HeaderInterface;
 ...
 */
class Request extends \S3IO\Aws2\Guzzle\Http\Message\AbstractMessage implements \S3IO\Aws2\Guzzle\Http\Message\RequestInterface
{
    ...
    public function __construct($method, $url, $headers = array())
    {
        ...
        $this->curlOptions = new \S3IO\Aws2\Guzzle\Common\Collection();
        $this->setUrl($url);
        if ($headers) {
    ...
    public function __clone()
    {
        ...
        $this->setState(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface::STATE_NEW);
     ...
     */
    public static function onRequestError(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
        $e = \S3IO\Aws2\Guzzle\Http\Exception\BadResponseException::factory($event['request'], $event['response']);
        $event['request']->setState(self::STATE_ERROR, array('exception' => $e) + $event->toArray());
        throw $e;
    }
    public function setClient(\S3IO\Aws2\Guzzle\Http\ClientInterface $client)
    {
        $this->client = $client;
        return $this;
    }
    ...
    public function setUrl($url)
    {
        if ($url instanceof Url) {
            ...
            $this->url = \S3IO\Aws2\Guzzle\Http\Url::factory($url);
        }
    ...
    {
        if (!$this->client) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\RuntimeException('A client must be set on the request');
        }
    ...
    public function setAuth($user, $password = '', $scheme = CURLAUTH_BASIC)
    {
        ...
        if (!is_numeric($scheme)) {
            ...
            if (!isset($authMap[$scheme])) {
                throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException($scheme . ' is not a valid authentication type');
            }
    ...
    public function startResponse(\S3IO\Aws2\Guzzle\Http\Message\Response $response)
    {
    ...
    public function setResponse(\S3IO\Aws2\Guzzle\Http\Message\Response $response, $queued = false)
    {
    ...
    public function setResponseBody($body)
    {
        ...
        if (is_string($body)) {
            ...
            if (!($body = fopen($body, 'w+'))) {
                throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Could not open ' . $body . ' for writing');
            }
            // @codeCoverageIgnoreEnd
        }
        $this->responseBody = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($body);
        return $this;
    }
    public function getResponseBody()
    {
        if ($this->responseBody === null) {
            $this->responseBody = \S3IO\Aws2\Guzzle\Http\EntityBody::factory()->setCustomData('default', true);
        }
    ...
    {
        \S3IO\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use $request->getResponseBody()->isRepeatable()');
        return !$this->responseBody ? true : $this->responseBody->isRepeatable();
    }
    public function getCookies()
    {
        if ($cookie = $this->getHeader('Cookie')) {
            $data = \S3IO\Aws2\Guzzle\Parser\ParserRegistry::getInstance()->getParser('cookie')->parseCookie($cookie);
            return $data['cookies'];
        }
        return array();
    }
    ...
    {
        if (!$this->eventDispatcher) {
            $this->setEventDispatcher(new \S3IO\Aws2\Symfony\Component\EventDispatcher\EventDispatcher());
        }
    ...
    protected function processResponse(array $context = array())
    {
        if (!$this->response) {
            ...
            $e = new \S3IO\Aws2\Guzzle\Http\Exception\RequestException('Error completing request');
            $e->setRequest($this);
            throw $e;
        }
        ...
        if ($this->state == \S3IO\Aws2\Guzzle\Http\Message\RequestInterface::STATE_COMPLETE) {
            ...
            if ($this->response->isError()) {
                $event = new \S3IO\Aws2\Guzzle\Common\Event($this->getEventArray());
    ...
    {
        \S3IO\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use Guzzle\\Plugin\\Cache\\DefaultCanCacheStrategy.');
        if (class_exists('S3IO\\Aws2\\Guzzle\\Plugin\\Cache\\DefaultCanCacheStrategy')) {
            $canCache = new \S3IO\Aws2\Guzzle\Plugin\Cache\DefaultCanCacheStrategy();
    ...
    {
        \S3IO\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the HistoryPlugin to track this.');
