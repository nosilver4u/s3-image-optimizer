 ...
 */
class EntityEnclosingRequest extends \S3IO\Aws2\Guzzle\Http\Message\Request implements \S3IO\Aws2\Guzzle\Http\Message\EntityEnclosingRequestInterface
{
    ...
    {
        $this->postFields = new \S3IO\Aws2\Guzzle\Http\QueryString();
    ...
    {
        $this->body = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($body);
        ...
        if ($size !== null && $size !== false) {
            ...
            if ('1.1' == $this->protocolVersion) {
                ...
                throw new \S3IO\Aws2\Guzzle\Http\Exception\RequestException('Cannot determine Content-Length and cannot use chunked Transfer-Encoding when using HTTP/1.0');
            }
        }
        return $this;
    }
    ...
    {
        $this->getParams()->set(\S3IO\Aws2\Guzzle\Http\RedirectPlugin::STRICT_REDIRECTS, $strict);
        if ($maxRedirects == 0) {
            $this->getParams()->set(\S3IO\Aws2\Guzzle\Http\RedirectPlugin::DISABLE, true);
        } else {
            $this->getParams()->set(\S3IO\Aws2\Guzzle\Http\RedirectPlugin::MAX_REDIRECTS, $maxRedirects);
        }
        return $this;
    }
    ...
    public function addPostFile($field, $filename = null, $contentType = null, $postname = null)
    {
        ...
        if ($field instanceof PostFileInterface) {
            ...
            throw new \S3IO\Aws2\Guzzle\Http\Exception\RequestException('The path to a file must be a string');
        } elseif (!empty($filename)) {
            // Adding an empty file will cause cURL to error out
            $data = new \S3IO\Aws2\Guzzle\Http\Message\PostFile($field, $filename, $contentType, $postname);
        }
        if ($data) {
    ...
    public function addPostFiles(array $files)
    {
        foreach ($files as $key => $file) {
            if ($file instanceof PostFileInterface) {
                ...
                throw new \S3IO\Aws2\Guzzle\Http\Exception\RequestException('File must be a string or instance of PostFileInterface');
            }
        }
        return $this;
    }
