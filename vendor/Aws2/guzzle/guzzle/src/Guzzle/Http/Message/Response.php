 ...
 */
class Response extends \S3IO\Aws2\Guzzle\Http\Message\AbstractMessage implements \Serializable
{
    /**
    ...
    {
        $data = \S3IO\Aws2\Guzzle\Parser\ParserRegistry::getInstance()->getParser('message')->parseResponse($message);
        if (!$data) {
            return false;
        }
    ...
    public function __construct($statusCode, $headers = null, $body = null)
    {
        ...
        $this->body = \S3IO\Aws2\Guzzle\Http\EntityBody::factory($body !== null ? $body : '');
        if ($headers) {
            if (is_array($headers)) {
                ...
                throw new \S3IO\Aws2\Guzzle\Http\Exception\BadResponseException('Invalid headers argument received');
            }
        }
    }
    /**
    ...
    public function json()
    {
        ...
        if (JSON_ERROR_NONE !== json_last_error()) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\RuntimeException('Unable to parse response body into JSON: ' . json_last_error());
        }
    ...
    public function xml()
    {
        ...
        if ($errorMessage) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\RuntimeException('Unable to parse response body into XML: ' . $errorMessage);
        }
        return $xml;
    }
    /**
    ...
    {
        return (int) $this->params->get(\S3IO\Aws2\Guzzle\Http\RedirectPlugin::REDIRECT_COUNT);
    }
    /**
    ...
    {
        \S3IO\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the HistoryPlugin.');
        return null;
    }
    /**
     * @deprecated
