...
use S3IO\Aws2\Guzzle\Batch\BatchClosureTransfer;
 ...
 * @codeCoverageIgnore
 */
class ResourceIteratorApplyBatched extends \S3IO\Aws2\Guzzle\Common\AbstractHasDispatcher
{
    /** @var callable|array */
    protected $callback;
     ...
     */
    public function __construct(\S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorInterface $iterator, $callback)
    {
    ...
    public function apply($perBatch = 50)
    {
        ...
        $batch = \S3IO\Aws2\Guzzle\Batch\BatchBuilder::factory()->createBatchesWith(new \S3IO\Aws2\Guzzle\Batch\BatchSizeDivisor($perBatch))->transferWith(new \S3IO\Aws2\Guzzle\Batch\BatchClosureTransfer(function (array $batch) use($that, $callback, &$batches, $it) {
            $batches++;
