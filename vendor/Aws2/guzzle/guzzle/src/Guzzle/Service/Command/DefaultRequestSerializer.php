 ...
 */
class DefaultRequestSerializer implements \S3IO\Aws2\Guzzle\Service\Command\RequestSerializerInterface
{
    ...
    {
        if (!self::$instance) {
            self::$instance = new self(\S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight::getInstance());
        }
     ...
     */
    public function __construct(\S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight $factory)
    {
     ...
     * @return self
     */
    public function addVisitor($location, \S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\Request\RequestVisitorInterface $visitor)
    {
     ...
     */
    protected function prepareAdditionalParameters(\S3IO\Aws2\Guzzle\Service\Description\OperationInterface $operation, \S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, \S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Guzzle\Service\Description\Parameter $additional)
    {
     ...
     */
    protected function createRequest(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command)
    {
        $operation = $command->getOperation();
        $client = $command->getClient();
        $options = $command[\S3IO\Aws2\Guzzle\Service\Command\AbstractCommand::REQUEST_OPTIONS] ?: array();
