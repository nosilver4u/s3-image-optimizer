...
use S3IO\Aws2\Guzzle\Service\Description\ValidatorInterface;
use S3IO\Aws2\Guzzle\Service\Description\SchemaValidator;
use S3IO\Aws2\Guzzle\Service\Exception\CommandException;
 ...
 */
abstract class AbstractCommand extends \S3IO\Aws2\Guzzle\Common\Collection implements \S3IO\Aws2\Guzzle\Service\Command\CommandInterface
{
     ...
     */
    public function __construct($parameters = array(), \S3IO\Aws2\Guzzle\Service\Description\OperationInterface $operation = null)
    {
        ...
        if (!$headers instanceof Collection) {
            $this[self::HEADERS_OPTION] = new \S3IO\Aws2\Guzzle\Common\Collection((array) $headers);
        }
    ...
    public function setOnComplete($callable)
    {
        if (!is_callable($callable)) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException('The onComplete function must be callable');
        }
    ...
    {
        if (!$this->client) {
            throw new \S3IO\Aws2\Guzzle\Service\Exception\CommandException('A client must be associated with the command before it can be executed.');
        }
    ...
    public function setClient(\S3IO\Aws2\Guzzle\Service\ClientInterface $client)
    {
        $this->client = $client;
        return $this;
    }
    ...
    {
        if (!$this->request) {
            throw new \S3IO\Aws2\Guzzle\Service\Exception\CommandException('The command must be prepared before retrieving the request');
        }
        return $this->request;
    }
    ...
    public function prepare()
    {
        if (!$this->isPrepared()) {
            if (!$this->client) {
                throw new \S3IO\Aws2\Guzzle\Service\Exception\CommandException('A client must be associated with the command before it can be prepared.');
            }
            ...
            if ($options = $this[\S3IO\Aws2\Guzzle\Service\Client::CURL_OPTIONS]) {
                $this->request->getCurlOptions()->overwriteWith(\S3IO\Aws2\Guzzle\Http\Curl\CurlHandle::parseCurlConfig($options));
            }
     ...
     * @return self
     */
    public function setValidator(\S3IO\Aws2\Guzzle\Service\Description\ValidatorInterface $validator)
    {
    ...
    {
        return new \S3IO\Aws2\Guzzle\Service\Description\Operation(array('name' => get_class($this)));
    }
    /**
    ...
    {
        $this->result = $this[self::RESPONSE_PROCESSING] != self::TYPE_RAW ? \S3IO\Aws2\Guzzle\Service\Command\DefaultResponseParser::getInstance()->parse($this) : $this->request->getResponse();
    }
    /**
    ...
    protected function validate()
    {
        ...
        if (!empty($errors)) {
            $e = new \S3IO\Aws2\Guzzle\Service\Exception\ValidationException('Validation errors: ' . implode("\n", $errors));
            $e->setErrors($errors);
            throw $e;
        }
    }
    ...
    {
        if (!$this->validator) {
            $this->validator = \S3IO\Aws2\Guzzle\Service\Description\SchemaValidator::getInstance();
        }
