...
use S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight;
...
use S3IO\Aws2\Guzzle\Service\Description\OperationInterface;
use S3IO\Aws2\Guzzle\Service\Description\Operation;
use S3IO\Aws2\Guzzle\Service\Exception\ResponseClassException;
 ...
 */
class OperationResponseParser extends \S3IO\Aws2\Guzzle\Service\Command\DefaultResponseParser
{
    ...
    {
        if (!static::$instance) {
            static::$instance = new static(\S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight::getInstance());
        }
     ...
     */
    public function __construct(\S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight $factory, $schemaInModels = false)
    {
     ...
     * @return self
     */
    public function addVisitor($location, \S3IO\Aws2\Guzzle\Service\Command\LocationVisitor\Response\ResponseVisitorInterface $visitor)
    {
        $this->factory->addResponseVisitor($location, $visitor);
        return $this;
    }
    protected function handleParsing(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, \S3IO\Aws2\Guzzle\Http\Message\Response $response, $contentType)
    {
        $operation = $command->getOperation();
        $type = $operation->getResponseType();
        $model = null;
        if ($type == \S3IO\Aws2\Guzzle\Service\Description\OperationInterface::TYPE_MODEL) {
            $model = $operation->getServiceDescription()->getModel($operation->getResponseClass());
        } elseif ($type == \S3IO\Aws2\Guzzle\Service\Description\OperationInterface::TYPE_CLASS) {
        ...
        } elseif ($command[\S3IO\Aws2\Guzzle\Service\Command\AbstractCommand::RESPONSE_PROCESSING] != \S3IO\Aws2\Guzzle\Service\Command\AbstractCommand::TYPE_MODEL) {
            // Returns a model with no visiting if the command response processing is not model
            return new \S3IO\Aws2\Guzzle\Service\Resource\Model(parent::handleParsing($command, $response, $contentType));
        } else {
            // Only inject the schema into the model if "schemaInModel" is true
            return new \S3IO\Aws2\Guzzle\Service\Resource\Model($this->visitResult($model, $command, $response), $this->schemaInModels ? $model : null);
        }
    }
    /**
     ...
     */
    protected function parseClass(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command)
    {
        // Emit the operation.parse_class event. If a listener injects a 'result' property, then that will be the result
        $event = new \S3IO\Aws2\Guzzle\Service\Command\CreateResponseClassEvent(array('command' => $command));
        ...
        if (!method_exists($className, 'fromCommand')) {
            throw new \S3IO\Aws2\Guzzle\Service\Exception\ResponseClassException("{$className} must exist and implement a static fromCommand() method");
        }
     ...
     */
    protected function visitResult(\S3IO\Aws2\Guzzle\Service\Description\Parameter $model, \S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, \S3IO\Aws2\Guzzle\Http\Message\Response $response)
    {
    ...
    protected function visitAdditionalProperties(\S3IO\Aws2\Guzzle\Service\Description\Parameter $model, \S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command, \S3IO\Aws2\Guzzle\Http\Message\Response $response, \S3IO\Aws2\Guzzle\Service\Description\Parameter $additional, &$result, array &$foundVisitors)
    {
