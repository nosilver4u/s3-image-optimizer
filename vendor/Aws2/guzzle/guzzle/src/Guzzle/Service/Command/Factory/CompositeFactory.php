 ...
 */
class CompositeFactory implements \IteratorAggregate, \Countable, \S3IO\Aws2\Guzzle\Service\Command\Factory\FactoryInterface
{
     ...
     * @return self
     */
    public static function getDefaultChain(\S3IO\Aws2\Guzzle\Service\ClientInterface $client)
    {
        $factories = array();
        if ($description = $client->getDescription()) {
            $factories[] = new \S3IO\Aws2\Guzzle\Service\Command\Factory\ServiceDescriptionFactory($description);
        }
        $factories[] = new \S3IO\Aws2\Guzzle\Service\Command\Factory\ConcreteClassFactory($client);
     ...
     */
    public function add(\S3IO\Aws2\Guzzle\Service\Command\Factory\FactoryInterface $factory, $before = null)
    {
        $pos = null;
        if ($before) {
