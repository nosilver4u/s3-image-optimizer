...
use S3IO\Aws2\Guzzle\Service\AbstractConfigLoader;
use S3IO\Aws2\Guzzle\Service\Exception\DescriptionBuilderException;
/**
 * Loader for service descriptions
 */
class ServiceDescriptionLoader extends \S3IO\Aws2\Guzzle\Service\AbstractConfigLoader
{
    protected function build($config, array $options)
    {
        ...
        return new \S3IO\Aws2\Guzzle\Service\Description\ServiceDescription(array('apiVersion' => isset($config['apiVersion']) ? $config['apiVersion'] : null, 'baseUrl' => isset($config['baseUrl']) ? $config['baseUrl'] : null, 'description' => isset($config['description']) ? $config['description'] : null, 'operations' => $operations, 'models' => isset($config['models']) ? $config['models'] : null) + $config);
    }
    /**
    ...
    protected function resolveExtension($name, array &$op, array &$operations)
    {
        ...
        foreach ((array) $op['extends'] as $extendedCommand) {
            if (empty($operations[$extendedCommand])) {
                throw new \S3IO\Aws2\Guzzle\Service\Exception\DescriptionBuilderException("{$name} extends missing operation {$extendedCommand}");
            }
