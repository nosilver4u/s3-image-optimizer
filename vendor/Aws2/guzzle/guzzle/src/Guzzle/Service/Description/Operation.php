...
namespace S3IO\Aws2\Guzzle\Service\Description;

use S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException;
/**
 * Data object holding the information of an API command
 */
class Operation implements \S3IO\Aws2\Guzzle\Service\Description\OperationInterface
{
    /** @var string Default command class to use when none is specified */
    const DEFAULT_COMMAND_CLASS = 'S3IO\\Aws2\\Guzzle\\Service\\Command\\OperationCommand';
     ...
     */
    public function __construct(array $config = array(), \S3IO\Aws2\Guzzle\Service\Description\ServiceDescriptionInterface $description = null)
    {
        ...
        if ($this->parameters) {
            foreach ($this->parameters as $name => $param) {
                if ($param instanceof Parameter) {
                    ...
                    $param['name'] = $name;
                    $this->addParam(new \S3IO\Aws2\Guzzle\Service\Description\Parameter($param, $this->description));
                }
            }
        }
        if ($this->additionalParameters) {
            if ($this->additionalParameters instanceof Parameter) {
                ...
                $this->setadditionalParameters(new \S3IO\Aws2\Guzzle\Service\Description\Parameter($this->additionalParameters, $this->description));
            }
        }
    }
    ...
    public function setServiceDescription(\S3IO\Aws2\Guzzle\Service\Description\ServiceDescriptionInterface $description)
    {
        $this->description = $description;
        return $this;
    }
     ...
     * @return self
     */
    public function addParam(\S3IO\Aws2\Guzzle\Service\Description\Parameter $param)
    {
     ...
     * 'S3IO\\Aws2\\Guzzle\\Foo\\Baz', or 'MyModelName' (to reference a model by ID).
     *
    ...
    {
        static $types = array(self::TYPE_PRIMITIVE => true, self::TYPE_CLASS => true, self::TYPE_MODEL => true, self::TYPE_DOCUMENTATION => true);
        if (!isset($types[$responseType])) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException('responseType must be one of ' . implode(', ', array_keys($types)));
        }
