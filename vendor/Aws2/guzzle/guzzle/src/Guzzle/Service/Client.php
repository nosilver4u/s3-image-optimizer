...
namespace S3IO\Aws2\Guzzle\Service;

use S3IO\Aws2\Guzzle\Common\Collection;
use S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException;
use S3IO\Aws2\Guzzle\Common\Exception\BadMethodCallException;
use S3IO\Aws2\Guzzle\Common\Version;
use S3IO\Aws2\Guzzle\Inflection\InflectorInterface;
use S3IO\Aws2\Guzzle\Inflection\Inflector;
use S3IO\Aws2\Guzzle\Http\Client as HttpClient;
use S3IO\Aws2\Guzzle\Http\Exception\MultiTransferException;
use S3IO\Aws2\Guzzle\Service\Exception\CommandTransferException;
...
use S3IO\Aws2\Guzzle\Service\Command\Factory\FactoryInterface as CommandFactoryInterface;
use S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorClassFactory;
use S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorFactoryInterface;
 ...
 */
class Client extends \S3IO\Aws2\Guzzle\Http\Client implements \S3IO\Aws2\Guzzle\Service\ClientInterface
{
    ...
    {
        return array_merge(\S3IO\Aws2\Guzzle\Http\Client::getAllEvents(), array('client.command.create', 'command.before_prepare', 'command.after_prepare', 'command.before_send', 'command.after_send', 'command.parse_response'));
    }
    /**
    ...
    public function getCommand($name, array $args = array())
    {
        ...
        if (!($command = $this->getCommandFactory()->factory($name, $args))) {
            throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException("Command was not found matching {$name}");
        }
     ...
     * @return self
     */
    public function setCommandFactory(\S3IO\Aws2\Guzzle\Service\Command\Factory\FactoryInterface $factory)
    {
     ...
     * @return self
     */
    public function setResourceIteratorFactory(\S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorFactoryInterface $factory)
    {
    ...
    public function execute($command)
    {
        if ($command instanceof CommandInterface) {
            ...
            throw new \S3IO\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Command must be a command or array of commands');
        }
    }
    public function setDescription(\S3IO\Aws2\Guzzle\Service\Description\ServiceDescriptionInterface $service)
    {
        $this->serviceDescription = $service;
        if ($this->getCommandFactory() && $this->getCommandFactory() instanceof CompositeFactory) {
            $this->commandFactory->add(new \S3IO\Aws2\Guzzle\Service\Command\Factory\ServiceDescriptionFactory($service));
        }
     ...
     * @return self
     */
    public function setInflector(\S3IO\Aws2\Guzzle\Inflection\InflectorInterface $inflector)
    {
    ...
    {
        if (!$this->inflector) {
            $this->inflector = \S3IO\Aws2\Guzzle\Inflection\Inflector::getDefault();
        }
     ...
     */
    protected function prepareCommand(\S3IO\Aws2\Guzzle\Service\Command\CommandInterface $command)
    {
        ...
        $request->setState(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface::STATE_NEW);
    ...
    protected function executeMultiple($commands)
    {
        ...
        try {
            ...
            $e = \S3IO\Aws2\Guzzle\Service\Exception\CommandTransferException::fromMultiTransferException($failureException);
    ...
    protected function getResourceIteratorFactory()
    {
        if (!$this->resourceIteratorFactory) {
            ...
            $this->resourceIteratorFactory = new \S3IO\Aws2\Guzzle\Service\Resource\ResourceIteratorClassFactory(array("{$prefix}\\Iterator", "{$prefix}\\Model"));
        }
    ...
    protected function getCommandFactory()
    {
        if (!$this->commandFactory) {
            $this->commandFactory = \S3IO\Aws2\Guzzle\Service\Command\Factory\CompositeFactory::getDefaultChain($this);
        }
    ...
    {
        \S3IO\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated');
    }
}
