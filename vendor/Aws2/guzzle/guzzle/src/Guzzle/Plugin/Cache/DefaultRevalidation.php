 ...
 */
class DefaultRevalidation implements \S3IO\Aws2\Guzzle\Plugin\Cache\RevalidationInterface
{
     ...
     */
    public function __construct(\S3IO\Aws2\Guzzle\Plugin\Cache\CacheStorageInterface $cache, \S3IO\Aws2\Guzzle\Plugin\Cache\CanCacheStrategyInterface $canCache = null)
    {
        $this->storage = $cache;
        $this->canCache = $canCache ?: new \S3IO\Aws2\Guzzle\Plugin\Cache\DefaultCanCacheStrategy();
    }
    ...
    {
        if ($request->getMethod() != \S3IO\Aws2\Guzzle\Http\Message\RequestInterface::GET) {
            return false;
        }
     ...
     */
    protected function handleBadResponse(\S3IO\Aws2\Guzzle\Http\Exception\BadResponseException $e)
    {
     ...
     */
    protected function createRevalidationRequest(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Guzzle\Http\Message\Response $response)
    {
     ...
     */
    protected function handle200Response(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Guzzle\Http\Message\Response $validateResponse)
    {
     ...
     * @return bool Returns true if valid, false if invalid
     */
    protected function handle304Response(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, \S3IO\Aws2\Guzzle\Http\Message\Response $validateResponse, \S3IO\Aws2\Guzzle\Http\Message\Response $response)
    {
