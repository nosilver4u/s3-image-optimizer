...
use S3IO\Aws2\Guzzle\Http\Message\MessageInterface;
use S3IO\Aws2\Guzzle\Http\Message\Request;
use S3IO\Aws2\Guzzle\Http\Message\RequestInterface;
use S3IO\Aws2\Guzzle\Http\Message\Response;
/**
 * Default cache storage implementation
 */
class DefaultCacheStorage implements \S3IO\Aws2\Guzzle\Plugin\Cache\CacheStorageInterface
{
    /** @var string */
    protected $keyPrefix;
    ...
    {
        $this->cache = \S3IO\Aws2\Guzzle\Cache\CacheAdapterFactory::fromCache($cache);
    ...
    public function purge($url)
    {
        foreach (array('GET', 'HEAD', 'POST', 'PUT', 'DELETE') as $method) {
            $this->delete(new \S3IO\Aws2\Guzzle\Http\Message\Request($method, $url));
        }
    }
    public function fetch(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($match[4] < time()) {
            ...
            $response = new \S3IO\Aws2\Guzzle\Http\Message\Response($match[2], $match[1]);
            if ($match[3]) {
     ...
     * @return string
     */
    protected function getCacheKey(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     * @return string
     */
    protected function getBodyKey($url, \S3IO\Aws2\Guzzle\Http\EntityBodyInterface $body)
    {
     ...
     * @return array
     */
    private function persistHeaders(\S3IO\Aws2\Guzzle\Http\Message\MessageInterface $message)
    {
