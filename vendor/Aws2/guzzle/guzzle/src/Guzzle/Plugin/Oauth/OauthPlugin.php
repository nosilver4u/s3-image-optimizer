...
namespace S3IO\Aws2\Guzzle\Plugin\Oauth;
 ...
 */
class OauthPlugin implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    /**
    ...
    {
        $this->config = \S3IO\Aws2\Guzzle\Common\Collection::fromConfig($config, array('version' => '1.0', 'request_method' => self::REQUEST_METHOD_HEADER, 'consumer_key' => 'anonymous', 'consumer_secret' => 'anonymous', 'signature_method' => 'HMAC-SHA1', 'signature_callback' => function ($stringToSign, $key) {
     ...
     * @return string
     */
    public function getSignature(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $timestamp, $nonce)
    {
     ...
     * @return string
     */
    public function getStringToSign(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $timestamp, $nonce)
    {
        ...
        $url = \S3IO\Aws2\Guzzle\Http\Url::factory($request->getUrl())->setQuery('')->setFragment(null);
    ...
    {
        $params = new \S3IO\Aws2\Guzzle\Common\Collection(array('oauth_consumer_key' => $this->config['consumer_key'], 'oauth_nonce' => $nonce, 'oauth_signature_method' => $this->config['signature_method'], 'oauth_timestamp' => $timestamp));
     ...
     * @return array
     */
    public function getParamsToSign(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request, $timestamp, $nonce)
    {
     ...
     * @return string
     */
    public function generateNonce(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     * @return int
     */
    public function getTimestamp(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
