 ...
 */
class BackoffPlugin extends \S3IO\Aws2\Guzzle\Common\AbstractHasDispatcher implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    const DELAY_PARAM = \S3IO\Aws2\Guzzle\Http\Curl\CurlMultiInterface::BLOCKING;
     ...
     */
    public function __construct(\S3IO\Aws2\Guzzle\Plugin\Backoff\BackoffStrategyInterface $strategy = null)
    {
    ...
    {
        return new self(new \S3IO\Aws2\Guzzle\Plugin\Backoff\TruncatedBackoffStrategy($maxRetries, new \S3IO\Aws2\Guzzle\Plugin\Backoff\HttpBackoffStrategy($httpCodes, new \S3IO\Aws2\Guzzle\Plugin\Backoff\CurlBackoffStrategy($curlCodes, new \S3IO\Aws2\Guzzle\Plugin\Backoff\ExponentialBackoffStrategy()))));
    }
    ...
    {
        return array('request.sent' => 'onRequestSent', 'request.exception' => 'onRequestSent', \S3IO\Aws2\Guzzle\Http\Curl\CurlMultiInterface::POLLING_REQUEST => 'onRequestPoll');
    }
    /**
    ...
    public function onRequestSent(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
        ...
        if ($delay !== false) {
            ...
            $request->setState(\S3IO\Aws2\Guzzle\Http\Message\RequestInterface::STATE_TRANSFER);
     ...
     * @param Event $event
     */
    public function onRequestPoll(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
        $request = $event['request'];
