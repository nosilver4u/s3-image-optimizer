...
namespace S3IO\Aws2\Guzzle\Plugin\Async;
 ...
 */
class AsyncPlugin implements \S3IO\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     * @param Event $event
     */
    public function onBeforeSend(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
     ...
     * @param Event $event
     */
    public function onCurlProgress(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
     ...
     * @param Event $event
     */
    public function onRequestTimeout(\S3IO\Aws2\Guzzle\Common\Event $event)
    {
        if ($event['exception'] instanceof CurlException) {
            $event['request']->setResponse(new \S3IO\Aws2\Guzzle\Http\Message\Response(200, array('X-Guzzle-Async' => 'Did not wait for the response')));
        }
    }
    /**
