...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\S3IO\Aws3\Psr\Http\Message\RequestInterface $request, \S3IO\Aws3\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \S3IO\Aws3\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \S3IO\Aws3\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
     ...
     * @return string
     */
    private function headers(\S3IO\Aws3\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
