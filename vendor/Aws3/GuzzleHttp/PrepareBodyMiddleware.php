...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\S3IO\Aws3\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \S3IO\Aws3\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\S3IO\Aws3\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    /**
     * Add expect header
     *
     * @return void
     */
    private function addExpectHeader(\S3IO\Aws3\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
