 ...
 * @internal
 */
abstract class AbstractMonitoringMiddleware implements \S3IO\Aws3\Aws\ClientSideMonitoring\MonitoringMiddlewareInterface
{
    ...
    protected $region;
    protected $service;
    protected static function getAwsExceptionHeader(\S3IO\Aws3\Aws\Exception\AwsException $e, $headerName)
    {
    ...
    protected static function getResultHeader(\S3IO\Aws3\Aws\ResultInterface $result, $headerName)
    {
    ...
    public function __invoke(\S3IO\Aws3\Aws\CommandInterface $cmd, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $g = function ($value) use($eventData, $enabled) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                return \S3IO\Aws3\GuzzleHttp\Promise\rejection_for($value);
            }
            return $value;
        };
        return \S3IO\Aws3\GuzzleHttp\Promise\promise_for($handler($cmd, $request))->then($g, $g);
    }
    private function getClientId()
    {
        return $this->unwrappedOptions()->getClientId();
    }
    private function getNewEvent(\S3IO\Aws3\Aws\CommandInterface $cmd, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request)
    {
