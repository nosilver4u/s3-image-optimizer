 ...
 * @internal
 */
class ApiCallAttemptMonitoringMiddleware extends \S3IO\Aws3\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
    ...
    private static function getResultAttemptLatency(\S3IO\Aws3\Aws\ResultInterface $result)
    {
    ...
    private static function getResultDestinationIp(\S3IO\Aws3\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultDnsLatency(\S3IO\Aws3\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultHttpStatusCode(\S3IO\Aws3\Aws\ResultInterface $result)
    {
        return $result['@metadata']['statusCode'];
    }
    private static function getAwsExceptionAttemptLatency(\S3IO\Aws3\Aws\Exception\AwsException $e)
    {
    ...
    private static function getAwsExceptionErrorCode(\S3IO\Aws3\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorCode();
    }
    private static function getAwsExceptionMessage(\S3IO\Aws3\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorMessage();
    }
    private static function getAwsExceptionDestinationIp(\S3IO\Aws3\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['primary_ip'])) {
            return $attempt['primary_ip'];
    ...
    private static function getAwsExceptionDnsLatency(\S3IO\Aws3\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['namelookup_time'])) {
            return (int) floor($attempt['namelookup_time'] * 1000);
    ...
    private static function getAwsExceptionHttpStatusCode(\S3IO\Aws3\Aws\Exception\AwsException $e)
    {
     ...
     * {@inheritdoc}
     */
    protected function populateRequestEventData(\S3IO\Aws3\Aws\CommandInterface $cmd, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request, array $event)
    {
