...
use S3IO\Aws3\GuzzleHttp\Event\EndEvent;
use S3IO\Aws3\GuzzleHttp\Exception\ConnectException;
use S3IO\Aws3\GuzzleHttp\Exception\RequestException;
use S3IO\Aws3\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use S3IO\Aws3\GuzzleHttp\Promise;
use S3IO\Aws3\GuzzleHttp\Psr7\Response as Psr7Response;
use S3IO\Aws3\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\S3IO\Aws3\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \S3IO\Aws3\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\S3IO\Aws3\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof Psr7StreamInterface) {
                        $exception->getResponse()->setBody(\S3IO\Aws3\GuzzleHttp\Stream\Stream::factory(file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \S3IO\Aws3\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\S3IO\Aws3\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof Psr7StreamInterface ? new \S3IO\Aws3\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\S3IO\Aws3\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \S3IO\Aws3\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \S3IO\Aws3\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\S3IO\Aws3\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \S3IO\Aws3\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \S3IO\Aws3\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
