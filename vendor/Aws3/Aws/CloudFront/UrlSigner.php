...
namespace S3IO\Aws3\Aws\CloudFront;
...
class UrlSigner
{
    ...
    {
        $this->signer = new \S3IO\Aws3\Aws\CloudFront\Signer($keyPairId, $privateKey);
    }
    /**
    ...
    public function getSignedUrl($url, $expires = null, $policy = null)
    {
        ...
        $uri = new \S3IO\Aws3\GuzzleHttp\Psr7\Uri($scheme . '://' . $urlSections[1]);
        $query = \S3IO\Aws3\GuzzleHttp\Psr7\parse_query($uri->getQuery(), PHP_QUERY_RFC3986);
    ...
    private function createRtmpUrl(\S3IO\Aws3\Psr\Http\Message\UriInterface $uri)
    {
