...
class QueryParamBuilder
{
    ...
    public function __invoke(\S3IO\Aws3\Aws\Api\StructureShape $shape, array $params)
    {
        if (!$this->methods) {
    ...
    protected function format(\S3IO\Aws3\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
    ...
    protected function format_structure(\S3IO\Aws3\Aws\Api\StructureShape $shape, array $value, $prefix, &$query)
    {
        if ($prefix) {
            $prefix .= '.';
        }
    ...
    protected function format_map(\S3IO\Aws3\Aws\Api\MapShape $shape, array $value, $prefix, array &$query)
    {
        $vals = $shape->getValue();
        $keys = $shape->getKey();
        if (!$this->isFlat($shape)) {
    ...
    protected function format_blob(\S3IO\Aws3\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
        $query[$prefix] = base64_encode($value);
    }
    protected function format_timestamp(\S3IO\Aws3\Aws\Api\TimestampShape $shape, $value, $prefix, array &$query)
    {
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $query[$prefix] = \S3IO\Aws3\Aws\Api\TimestampShape::format($value, $timestampFormat);
    }
    protected function format_boolean(\S3IO\Aws3\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
