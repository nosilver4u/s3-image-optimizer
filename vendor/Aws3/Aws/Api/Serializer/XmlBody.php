...
class XmlBody
{
     ...
     * @return string
     */
    public function build(\S3IO\Aws3\Aws\Api\Shape $shape, array $args)
    {
        $xml = new \XMLWriter();
        $xml->openMemory();
    ...
    private function startElement(\S3IO\Aws3\Aws\Api\Shape $shape, $name, \XMLWriter $xml)
    {
    ...
    private function format(\S3IO\Aws3\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
    ...
    private function defaultShape(\S3IO\Aws3\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->text($value);
        $xml->endElement();
    }
    private function add_structure(\S3IO\Aws3\Aws\Api\StructureShape $shape, $name, array $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
    ...
    private function getStructureMembers(\S3IO\Aws3\Aws\Api\StructureShape $shape, array $value)
    {
        $members = [];
    ...
    private function add_list(\S3IO\Aws3\Aws\Api\ListShape $shape, $name, array $value, \XMLWriter $xml)
    {
    ...
    private function add_map(\S3IO\Aws3\Aws\Api\MapShape $shape, $name, array $value, \XMLWriter $xml)
    {
    ...
    private function add_blob(\S3IO\Aws3\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->writeRaw(base64_encode($value));
        $xml->endElement();
    }
    private function add_timestamp(\S3IO\Aws3\Aws\Api\TimestampShape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $xml->writeRaw(\S3IO\Aws3\Aws\Api\TimestampShape::format($value, $timestampFormat));
        $xml->endElement();
    }
    private function add_boolean(\S3IO\Aws3\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->writeRaw($value ? 'true' : 'false');
        $xml->endElement();
    }
    private function add_string(\S3IO\Aws3\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
