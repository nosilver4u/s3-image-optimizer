...
class S3EndpointMiddleware
{
    ...
    private function canAccelerate(\S3IO\Aws3\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \S3IO\Aws3\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\S3IO\Aws3\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyDualStackEndpoint(\S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\S3IO\Aws3\Aws\CommandInterface $command, $pattern)
    {
        return "{$command['Bucket']}.{$pattern}.amazonaws.com";
    }
    private function getBucketlessPath($path, \S3IO\Aws3\Aws\CommandInterface $command)
    {
