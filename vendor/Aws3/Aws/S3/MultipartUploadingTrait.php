...
trait MultipartUploadingTrait
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\S3IO\Aws3\Aws\S3\S3ClientInterface $client, $bucket, $key, $uploadId)
    {
        $state = new \S3IO\Aws3\Aws\Multipart\UploadState(['Bucket' => $bucket, 'Key' => $key, 'UploadId' => $uploadId]);
        ...
        $state->setStatus(\S3IO\Aws3\Aws\Multipart\UploadState::INITIATED);
        return $state;
    }
    protected function handleResult(\S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Aws\ResultInterface $result)
    {
        $this->getState()->markPartAsUploaded($command['PartNumber'], ['PartNumber' => $command['PartNumber'], 'ETag' => $this->extractETag($result)]);
    }
    protected abstract function extractETag(\S3IO\Aws3\Aws\ResultInterface $result);
    ...
    protected function determinePartSize()
    {
        ...
        $partSize = $this->getConfig()['part_size'] ?: \S3IO\Aws3\Aws\S3\MultipartUploader::PART_MIN_SIZE;
        // Adjust the part size to be larger for known, x-large uploads.
        if ($sourceSize = $this->getSourceSize()) {
            $partSize = (int) max($partSize, ceil($sourceSize / \S3IO\Aws3\Aws\S3\MultipartUploader::PART_MAX_NUM));
        }
        // Ensure that the part size follows the rules: 5 MB <= size <= 5 GB.
        if ($partSize < \S3IO\Aws3\Aws\S3\MultipartUploader::PART_MIN_SIZE || $partSize > \S3IO\Aws3\Aws\S3\MultipartUploader::PART_MAX_SIZE) {
