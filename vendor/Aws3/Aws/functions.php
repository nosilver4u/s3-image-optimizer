 ...
 * @throws \RuntimeException
 */
function serialize(\S3IO\Aws3\Aws\CommandInterface $command)
{
    $request = null;
    $handlerList = $command->getHandlerList();
    // Return a mock result.
    $handlerList->setHandler(function (\S3IO\Aws3\Aws\CommandInterface $_, \S3IO\Aws3\Psr\Http\Message\RequestInterface $r) use(&$request) {
        $request = $r;
        return new \S3IO\Aws3\GuzzleHttp\Promise\FulfilledPromise(new \S3IO\Aws3\Aws\Result([]));
    });
