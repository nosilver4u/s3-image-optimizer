...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3, $additionalRetryConfig = [])
    {
        ...
        return function ($retries, \S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request, \S3IO\Aws3\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors, $additionalRetryConfig) {
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$monitoringEvents, &$g) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($retries, $command, $request, null, $value)) {
                    return \S3IO\Aws3\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
