...
class PartitionEndpointProvider
{
    ...
    public function __construct(array $partitions, $defaultPartition = 'aws', $options = [])
    {
        $this->partitions = array_map(function (array $definition) {
            return new \S3IO\Aws3\Aws\Endpoint\Partition($definition);
    ...
    {
        $data = \S3IO\Aws3\Aws\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        $prefixData = \S3IO\Aws3\Aws\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
    ...
    public static function mergePrefixData($data, $prefixData)
    {
        ...
        foreach ($data["partitions"] as $index => $partition) {
            foreach ($prefixGroups as $current => $old) {
                $serviceData = \S3IO\Aws3\JmesPath\Env::search("services.{$current}", $partition);
                if (!empty($serviceData)) {
                    foreach ($old as $prefix) {
                        if (empty(\S3IO\Aws3\JmesPath\Env::search("services.{$prefix}", $partition))) {
