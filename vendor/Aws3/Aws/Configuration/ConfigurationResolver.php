...
namespace S3IO\Aws3\Aws\Configuration;
...
class ConfigurationResolver
{
    ...
    public static function ini($key, $expectedType, $profile = null, $filename = null, $options = [])
    {
        ...
        $data = @\S3IO\Aws3\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
    ...
    {
        if ($type === 'bool' && !is_null($convertedValue = \S3IO\Aws3\Aws\boolean_value($value))) {
    ...
    private static function retrieveValueFromIniSubsection($data, $profile, $filename, $expectedType, $options)
    {
        ...
        $services_section = \S3IO\Aws3\Aws\parse_ini_section_with_subsections($filename, "services {$data[$profile]['services']}");
