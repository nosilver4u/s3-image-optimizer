...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'S3IO\\Aws3\\Aws\\Exception\\AwsException', $collectStats = false)
    {
    ...
    public function __invoke(\S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \S3IO\Aws3\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\S3IO\Aws3\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \S3IO\Aws3\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\S3IO\Aws3\Aws\CommandInterface $command, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request, \S3IO\Aws3\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \S3IO\Aws3\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \S3IO\Aws3\Psr\Http\Message\RequestInterface $request, \S3IO\Aws3\Aws\CommandInterface $command, array $stats)
    {
