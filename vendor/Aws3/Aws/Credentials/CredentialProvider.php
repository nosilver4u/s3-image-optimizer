...
use S3IO\Aws3\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (!empty(getenv(\S3IO\Aws3\Aws\Credentials\EcsCredentialProvider::ENV_URI))) {
     ...
     * @return callable
     */
    public static function fromCredentials(\S3IO\Aws3\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \S3IO\Aws3\GuzzleHttp\Promise\promise_for($creds);
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\S3IO\Aws3\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return new \S3IO\Aws3\GuzzleHttp\Promise\RejectedPromise($reason);
            });
        };
    }
    /**
     ...
     * @return callable
     */
    public static function cache(callable $provider, \S3IO\Aws3\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof CredentialsInterface && !$found->isExpired()) {
                return \S3IO\Aws3\GuzzleHttp\Promise\promise_for($found);
            }
            return $provider()->then(function (\S3IO\Aws3\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \S3IO\Aws3\GuzzleHttp\Promise\promise_for(new \S3IO\Aws3\Aws\Credentials\Credentials($key, $secret, getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \S3IO\Aws3\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    {
        return new \S3IO\Aws3\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \S3IO\Aws3\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \S3IO\Aws3\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \S3IO\Aws3\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \S3IO\Aws3\GuzzleHttp\Promise\promise_for(new \S3IO\Aws3\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \S3IO\Aws3\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \S3IO\Aws3\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \S3IO\Aws3\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \S3IO\Aws3\GuzzleHttp\Promise\promise_for(new \S3IO\Aws3\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient)
    {
        ...
        if (empty($stsClient)) {
            ...
            $sourceCredentials = call_user_func(\S3IO\Aws3\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            $stsClient = new \S3IO\Aws3\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $creds = $stsClient->createCredentials($result);
        return \S3IO\Aws3\GuzzleHttp\Promise\promise_for($creds);
    }
    /**
    ...
    {
        $profileData = \S3IO\Aws3\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \S3IO\Aws3\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (file_exists($credFile)) {
            $profiles = \S3IO\Aws3\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }
        if (file_exists($configFile)) {
            $configProfileData = \S3IO\Aws3\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    {
        return new \S3IO\Aws3\GuzzleHttp\Promise\RejectedPromise(new \S3IO\Aws3\Aws\Exception\CredentialsException($msg));
    }
}
