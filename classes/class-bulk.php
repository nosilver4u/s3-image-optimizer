...
class Bulk extends Base {
	...
	public function remove_from_table() {
		...
		if ( $this->table_delete_image( (int) $_POST['s3io_image_id'] ) ) {
			echo '1';
		}
		die();
	}
	 ...
	 */
	protected function table_update( $path, $opt_size, $orig_size, $results_msg, $id = false, $bucket = '' ) {
		global $wpdb;
		...
		if ( $opt_size >= $orig_size ) {
			...
			if ( $id ) {
				...
				if ( $optimized_query && ! empty( $optimized_query['results'] ) ) {
					$this->debug_message( "s3io: found already optimized $id" );
			...
			$savings_str = $this->size_format( $savings, 1 );
			// Replace spaces with html entity encoding.
		...
		if ( ! empty( $already_optimized['results'] ) && $opt_size === $orig_size ) {
			$this->debug_message( 'returning results without update' );
		...
		if ( ! empty( $already_optimized['id'] ) ) {
			...
			if ( $updated ) {
				$this->debug_message( "updated results for $path" );
				$wpdb->flush();
				return $results_msg;
			}
		}
	...
	public function bulk_init() {
		...
		update_option( 's3io_last_run', array( time(), $this->table_count_pending() ) );
	 ...
	 */
	public function bulk_loop( $auto = false, $verbose = false ) {
		$this->debug_message( '<b>' . __METHOD__ . '()</b>' );
		$output = array();
		// verify that an authorized user has started the optimizer.
		$permissions = apply_filters( 'ewww_image_optimizer_bulk_permissions', '' );
		...
		try {
			...
			die( wp_json_encode( array( 'error' => wp_kses_post( $this->format_aws_exception( $e->getMessage() ) ) ) ) );
		}
			...
			if ( defined( 'WP_CLI' ) && WP_CLI ) {
				WP_CLI::error(
					sprintf(
						...
						__( 'Fetch failed for bucket: %1$s, path: %2$s, message: %3$s', 's3-image-optimizer' ),
						$image_record['bucket'],
						$image_record['path'],
						$e->getMessage()
					)
		...
		if ( $verbose && defined( 'WP_CLI' ) && WP_CLI ) {
			...
			WP_CLI::line( sprintf( esc_html__( 'Starting optimization for %s', 's3-image-optimizer' ), $filename ) );
		}
		...
		if ( $verbose && defined( 'WP_CLI' ) && WP_CLI ) {
			/* translators: %s: filename */
			WP_CLI::line( sprintf( esc_html__( 'Optimized %s', 's3-image-optimizer' ), $filename ) );
		}
		...
		if ( $new_size && $new_size < $fetch_result['ContentLength'] ) {
			if ( $verbose && defined( 'WP_CLI' ) && WP_CLI ) {
				/* translators: %s: filename */
				WP_CLI::line( sprintf( esc_html__( 'Starting re-upload for %s', 's3-image-optimizer' ), $filename ) );
			}
			...
			try {
				...
				if ( defined( 'WP_CLI' ) && WP_CLI ) {
					WP_CLI::error(
						sprintf(
							/* translators: 1: bucket name, 2: path to file, 3: error message */
							__( 'Upload failed for bucket: %1$s, path: %2$s, message: %3$s', 's3-image-optimizer' ),
							$image_record['bucket'],
							$image_record['path'],
							$e->getMessage()
						)
			...
			if ( $verbose && defined( 'WP_CLI' ) && WP_CLI ) {
				/* translators: %s: filename */
				WP_CLI::line( sprintf( esc_html__( 'Finished re-upload of %s', 's3-image-optimizer' ), $filename ) );
			}
		}
		unlink( $filename );
		...
		if ( $webp_size && ! empty( $webp_key ) ) {
			if ( $verbose && defined( 'WP_CLI' ) && WP_CLI ) {
				/* translators: %s: filename */
				WP_CLI::line( sprintf( esc_html__( 'Uploading %s', 's3-image-optimizer' ), $webp_filename ) );
			}
		...
		$this->table_update( $image_record['path'], $new_size, $fetch_result['ContentLength'], $results[1], $image_record['id'], $image_record['bucket'] );
		...
		if ( ! $auto ) {
			...
			/* translators: %s: time in seconds */
			WP_CLI::line( sprintf( __( 'Elapsed: %s seconds', 's3-image-optimizer' ), number_format_i18n( $elapsed, 1 ) ) );
		}
	}
	 ...
	 */
	public function bulk_cleanup( $auto = false ) {
		$this->debug_message( '<b>' . __METHOD__ . '()</b>' );
		// Verify that an authorized user has started the optimizer.
		$permissions = apply_filters( 'ewww_image_optimizer_bulk_permissions', '' );
		if ( ! $auto && ( empty( $_REQUEST['s3io_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['s3io_wpnonce'] ), 's3io-bulk' ) || ! current_user_can( $permissions ) ) ) {
	...
	public function url_loop() {
		...
		$upload_dir = trailingslashit( $upload_dir ) . trailingslashit( sanitize_file_name( $url_args['bucket'] ) );
		$this->debug_message( "stashing files in $upload_dir" );
		if ( false !== strpos( $upload_dir, 's3://' ) ) {
			wp_die(
				wp_json_encode(
		...
		$this->table_update( $url_args['path'], $new_size, $fetch_result['ContentLength'], $results[1], false, $url_args['bucket'] );
		// Make sure ewww doesn't keep a record of these files.
		global $wpdb;
		$wpdb->delete( $wpdb->ewwwio_images, array( 'path' => ewww_image_optimizer_relativize_path( $filename ) ) );
		if ( function_exists( 'ewww_image_optimizer_debug_log' ) ) {
